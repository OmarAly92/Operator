...
// TestAttachmentStreamsRealPtyHostPane attaches a real PTY to a real
// pty-host session and asserts output streams back, then that killing the
// session stops the attachment without a re-attach storm.
func TestAttachmentStreamsRealPtyHostPane(t *testing.T) {
	realpty.IsolateRegistry(t)
	rt := realpty.Runtime(t)

	name := "opr-term-it-" + strconv.Itoa(os.Getpid())
	handle, err := rt.Create(context.Background(), ports.RuntimeConfig{
		SessionID:     domain.SessionID(name),
		WorkspacePath: t.TempDir(),
		Argv:          []string{"sh", "-lc", "printf OPERATOR_READY\\n; exec sh -i"},
	})
	if err != nil {
	...
	eventually(t, 3*time.Second, func() bool { return a.writeLeased([]byte("echo OPERATOR_MARKER_42\n"), nil) == nil })
	eventually(t, 5*time.Second, func() bool { return strings.Contains(got.string(), "OPERATOR_MARKER_42") })
...
// remount/reconnect flow). B's attach must adopt B's size, not A's.
func TestAttachmentReattachAdoptsNewSize(t *testing.T) {
	realpty.IsolateRegistry(t)
	rt := realpty.Runtime(t)

	name := "opr-term-size-it-" + strconv.Itoa(os.Getpid())
	handle, err := rt.Create(context.Background(), ports.RuntimeConfig{
		SessionID:     domain.SessionID(name),
		WorkspacePath: t.TempDir(),
		Argv:          []string{"sh", "-lc", "printf OPERATOR_READY\\n; exec sh -i"},
	...
	// grid (assert cols land near 148 and far from 115).
	...
	// keystroke can be dropped; retrying covers that. On timeout we dump exactly
	// what the pane produced so the failure is self-explaining (e.g. the probe
	// echoed but never executed, or stty errored).
	var captured string
	gotWidth := false
	...
	for time.Now().Before(deadline) {
		...
		if cols > 130 { // B's 148 minus any chrome; a stale A-layout reports <=115
			gotWidth = true
			break
		}
	}
