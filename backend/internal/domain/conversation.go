...
// are authoritative for what Operator renders and for delivery state — Operator never
...
// its own terminal status when a turn is cancelled, and Operator must not relabel it.
const (
...
type ConversationRecord struct {
	...
	// when the user has overridden the Operator label, because it is the name the
	...
	// AppliedTitle is the last provider title Operator wrote into the session's display
	// name. It is what makes "replace a label Operator chose" distinguishable from
	// "overwrite a label a person chose". Empty means Operator has never named it.
...
//
// Durable because it is a correction to a claim Operator has already made. The composer
...
type ConversationAccount struct {
	...
	PlanLabel string `json:"planLabel,omitempty"`
	// ReauthRequiredAt is when the provider last asked for credentials Operator does not
...
// ThreadStatus is the provider's own lifecycle state for a thread. It is NOT Operator's
...
// a value Operator does not recognize is stored as-is rather than being flattened into
// something Operator does understand.
const (
...
type ConversationThreadState struct {
	...
	// on: Operator has never observed this notification, so tearing a controller down on
...
// Plan step statuses. Operator's own snake_case spelling, not the provider's camelCase:
...
// ConversationSettings are the per-turn provider choices Operator remembers.
//
// Durable rather than client-side because they must apply to turns Operator dispatches
// on the user's behalf: a queued message draining after a restart, a relay from
// `opr send`. A preference held in the renderer would quietly stop applying the
...
type ConversationSettings struct {
	...
	// ApprovalMode is Operator's permission vocabulary, applied per turn.
...
type ConversationTurn struct {
	ID             string `json:"id"`
	ConversationID string `json:"conversationId"`
	// HandledBySessionID is the Operator session whose controller ran the turn. For a
	...
	// row survives because Operator does not destroy durable facts, but the agent no
...
type ConversationTurnDiff struct {
	...
	// Truncated reports that the file list was cut at Operator's cap. A cut list shown
...
type ConversationActivity struct {
	...
	// CommandOutputTruncated reports that accumulation stopped at Operator's cap.
	...
	// StreamedTextTruncated reports that accumulation stopped at Operator's cap.
