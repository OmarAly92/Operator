...
func TestPrepareEnvironmentUsesPrivateOperatorOwnedDirectoriesAndImmutableManifest(t *testing.T) {
	dataDir := t.TempDir()
...
func TestPrepareHostTrustedEnvironmentUsesPrivateOperatorOwnedDirectories(t *testing.T) {
	dataDir := t.TempDir()
	...
	for _, path := range []string{env.Root, env.WorkingDirectory, env.ConfigRoot, env.StateRoot, env.CacheRoot, env.TempRoot} {
		...
		if relErr != nil || strings.HasPrefix(rel, "..") || statErr != nil {
			t.Fatalf("private Operator-owned directory %q: rel=%q relErr=%v statErr=%v", path, rel, relErr, statErr)
		}
		if info.Mode().Perm() != 0o700 {
			t.Fatalf("private Operator-owned directory %q mode = %v", path, info.Mode().Perm())
		}
	}
	...
	if _, err := PrepareHostTrustedEnvironment("relative", "review-worker-1"); err == nil {
		t.Fatal("relative Operator data directory accepted")
	}
...
func TestPrepareEnvironmentRequiresAbsoluteDataDir(t *testing.T) {
	if _, err := PrepareEnvironment("relative", testManifest(t)); err == nil {
		t.Fatal("relative Operator data directory was accepted")
	}
}
