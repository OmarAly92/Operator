...
func (m *Manager) RestoreWithMode(ctx context.Context, id domain.SessionID, grid ports.PaneGrid) (RestoreResult, error) {
	...
	// unresumable worker: one with no task and no native id to resume.
...
// cleanupOne reclaims one terminated session's workspace. Split out of
// Cleanup's loop so per-session error handling doesn't complicate the loop
// body. Returns "" when the workspace was reclaimed; a non-empty reason means
...
func (m *Manager) buildSpawnTexts(ctx context.Context, cfg ports.SpawnConfig) (prompt, systemPrompt string, err error) {
	...
	systemPrompt, err = m.buildSystemPrompt(ctx, cfg.ProjectID)
	if err != nil {
		return "", "", err
	}
...
// buildSystemPrompt derives the standing instructions for a session from
...
func (m *Manager) prepareWorkspace(ctx context.Context, agent ports.Agent, id domain.SessionID, workspacePath, systemPrompt, systemPromptFile string, agentConfig ports.AgentConfig, env map[string]string) error {
	...
	if pl, ok := agent.(preLauncher); ok {
		if err := pl.PreLaunch(ctx, ports.LaunchConfig{DataDir: m.dataDir, SessionID: string(id), WorkspacePath: workspacePath, Env: env}); err != nil {
			m.cleanupPreparedAgentWorkspace(ctx, agent, id, workspacePath, env)
