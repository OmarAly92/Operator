// Package acp implements Operator's provider-neutral Chat driver over the Agent
...
// updates into Operator's durable conversation vocabulary.
package acp
...
// Launch describes one ACP agent process. Command may be either Operator's packaged
...
// LaunchConfig is the resolved Operator session context a provider binding may use to
...
type Config struct {
	...
	// SessionMeta carries adapter-defined ACP extensions whenever Operator creates the
	...
	// SessionMode maps Operator's approval vocabulary onto this ACP agent's mode ids.
	// Empty means "leave the provider/user default unchanged".
	SessionMode func(ports.PermissionMode) string
	// SessionOptions maps Operator's per-turn choices onto ACP config option ids.
...
// Harness identifies the Operator harness this ACP transport adapts.
...
// Start creates a new ACP session in the Operator worktree.
...
// session/load, Operator uses it to recover both provider context and the normalized
...
func (d *Driver) connect(
	...
	init, err := conv.conn.Initialize(initCtx, acpsdk.InitializeRequest{
		...
		ClientCapabilities: acpsdk.ClientCapabilities{
			...
			// not grant the agent access to Operator's terminal or filesystem APIs.
			Meta: map[string]any{
...
func conversationCapabilities(
	...
	// These are facilities Operator itself negotiated as the ACP client. An agent that
