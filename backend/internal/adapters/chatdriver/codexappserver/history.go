...
// refuse builds a refusal Operator decided on itself, for the cases where the provider
...
// providerTurn is the subset of a thread's turn list Operator reads.
...
// terminal visible in Operator's structured timeline as well; resuming the thread alone
...
//
// It changes what the agent remembers. Operator's rows have to follow, and that is the
...
func (c *conversation) Rollback(ctx context.Context, providerTurnID string) error {
	...
	if discard == 0 {
		...
		// no longer holds. Refusing beats guessing: any count Operator invented here would
...
//
// Read from the provider rather than counted from Operator's rows because the two sets
// legitimately differ: Operator records a turn whose dispatch the provider rejected, and
// a turn from before a resume may be absent here. thread/rollback counts in the
// provider's terms, so counting in Operator's would discard the wrong number of turns.
...
//
// Nothing is written to Operator's rows here. The provider answers a bare {} and then
// emits thread/name/updated, and that notification is the single path by which a
// title reaches Operator — including when the name was set by some other client
// entirely. Writing it optimistically as well would give Operator two sources for one
