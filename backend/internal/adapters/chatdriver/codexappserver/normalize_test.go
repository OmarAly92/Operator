...
// The provider reports a cancelled turn with its own terminal status. Operator must
...
// Operator persists the user's message when it accepts the send. The provider echoing
...
func TestNormalizeIgnoresProviderBookkeeping(t *testing.T) {
	for _, method := range []string{
		...
		// affects latency, not the conversation, and Operator has nothing to do with it.
		...
		"guardianWarning",
		// The client-driven exec API. Operator never asks the server to run anything, so an
		// agent tool call never arrives on these.
		"command/exec/outputDelta",
		"process/outputDelta",
		// The voice surface. Operator has none.
...
func TestUnwrapShellLeavesPlainCommands(t *testing.T) {
	for _, tc := range []struct{ in, want string }{
		...
		{"opr spawn --project p --name w", "opr spawn --project p --name w"},
...
// 0.146.0 never sends it. Reading only the notification would mean Operator silently
