...
// resolves would hang the session indefinitely. On expiry Operator refuses rather than
...
// Operator does not model risks consenting to something on the user's behalf.
...
// ProviderConversationID is the Codex thread id Operator persists for resume.
	...
	for n := range c.conn.notifs() {
		...
		for _, ev := range normalizeNotification(n, time.Now()) {
			...
			if ev.Kind == ports.ChatEventApprovalResolved {
				...
				// card Operator is still showing is stale.
...
// account, and gated by entitlement that Operator cannot see. A table in Operator would be
// wrong within a week.
...
//
// It only claims a number it actually has. Operator has no context figure until the
...
// Both halves matter: forwarding an invented decision is consent Operator made up, and
...
// reconstruct them. Operator echoes what the provider sent rather than rebuilding it.
...
// than answering immediately. Everything Operator does not model is refused with an
...
//
// Operator does not hold provider credentials. Codex owns its own ChatGPT OAuth tokens
...
// in a mode Operator does not use, which a test cannot arrange without breaking real auth.
func (c *conversation) reportAuthRefreshRequest(params json.RawMessage) error {
	...
	// asked for credentials Operator cannot supply.
	...
	c.log.Warn("app-server asked for ChatGPT auth tokens Operator does not hold", "reason", reason)
	c.emit(ports.ChatEvent{
...
// refuseDynamicToolCall declines a request to run a tool Operator never offered.
//
// `item/tool/call` asks the CLIENT to execute a tool the client declared during
// initialize. Operator declares none, so a well-behaved provider will never send this and
// one that does is asking Operator to run something it has no definition for. Refusing is
...
func (c *conversation) refuseDynamicToolCall(params json.RawMessage) error {
	...
	_ = json.Unmarshal(params, &p)
	c.log.Warn("refusing dynamic tool call: Operator declares no client-side tools",
...
// which Operator learns the outcome.
func (c *conversation) ReloadMCPServers(ctx context.Context) ([]ports.ChatMCPServer, error) {
	...
	if err := c.conn.request(ctx, codexproto.MethodMcpServerStatusList, map[string]any{
		...
		// handful of servers is hundreds of kilobytes Operator would immediately discard.
		"detail":   "summary",
		"threadId": c.threadID,
	}, &resp); err != nil {
...
// approvalPayload is the subset of an approval request Operator renders.
