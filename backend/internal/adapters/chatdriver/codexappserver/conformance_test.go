...
//
// Every method Operator sends or reads is named here. Two failure modes this catches
// that nothing else does:
//
//   - Operator calls a method the installed provider no longer declares, which arrives
//     as a generic -32601 at runtime and looks like a broken session.
//   - Operator reads a payload field that has been renamed, which unmarshals to a zero
...
var driverMethods = []struct {
	...
	// NOT thread/started: Operator reads the thread id from the thread/start RESPONSE, so
	...
	// Inbound request: the provider asking for ChatGPT credentials Operator does not hold.
	...
	// Inbound request: run a tool Operator never declared. Refused explicitly.
...
func TestGeneratedProtocolMatchesTheInstalledProvider(t *testing.T) {
	...
	// New provider methods are not a failure on their own — Operator does not have to use
	...
	// A method Operator's generated file has and the provider does not is the dangerous
...
// not yet published.
//
// So Operator handling none of it costs nothing today: there is no reachable behaviour
...
func TestRealtimeStaysUnreachableUntilTheProviderPublishesAnEntryPoint(t *testing.T) {
	...
	for method, direction := range methodDirectionsFromSchema(t, dir) {
		...
		// A ClientRequest or ClientNotification is something Operator can send. Either one
