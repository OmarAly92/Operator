...
// Coverage for the notifications Operator used to drop.
//
...
func TestNormalizeTurnPlanUpdated(t *testing.T) {
	...
	// The provider's camelCase becomes Operator's persisted spelling.
...
// could read as a status — and put a provider DTO on Operator's wire.
...
// A move arrives as an update carrying the destination. The path Operator shows must be
...
func TestNormalizeAutoApprovalReview(t *testing.T) {
	...
	if started.ProviderItemID != "opr-review-5b95c779" {
