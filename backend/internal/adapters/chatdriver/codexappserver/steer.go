...
//     `clientId` = the clientUserMessageId Operator sent). It arrived ~5s after the call
...
//   - In-flight work is NOT killed by the provider. The shell command Operator's turn had
...
//     refused with "no active turn to steer", even though Operator already held the right
...
// stronger guarantee than a lock Operator could hold here — a turn that ends mid-flight
...
func (c *conversation) Steer(
	...
	turn := resp.TurnID
	if turn == "" {
		// Every observed response carried it, but a steer whose target Operator cannot name
