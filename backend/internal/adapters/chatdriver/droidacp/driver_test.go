...
func TestConfigureUsesUserDroidACPDaemonAndStandingInstructions(t *testing.T) {
	args, env, err := configure(acpdriver.LaunchConfig{SystemPrompt: "Follow Operator orchestrator rules."})
	if err != nil {
		t.Fatalf("configure: %v", err)
	}
	want := []string{
		"exec", "--output-format", "acp-daemon",
		"--append-system-prompt", "Follow Operator orchestrator rules.",
	}
...
func TestConfigureReusesDroidRuntimeModelAndAutonomySettings(t *testing.T) {
	...
	settingsPath := filepath.Join(
		dataDir, "agent-runtime", "droid", "opr-droid-worker-1-settings.json")
	want := []string{
...
func TestConfigureRequiresOperatorDataDirForDroidRuntimeSettings(t *testing.T) {
	_, _, err := configure(acpdriver.LaunchConfig{Model: "claude-sonnet-5"})
	if err == nil {
		t.Fatal("configure succeeded without Operator data directory")
	}
}
