...
// processes so opr stop can find and graceful-kill them even when session
// metadata is lost. Ported from operator's windows-pty-registry.ts.
package ptyregistry
...
// registryFile resolves ~/.operator/windows-pty-hosts.json. Uses os.UserHomeDir()
// so t.Setenv("HOME", dir) in tests redirects reads/writes to a temp dir.
// ponytail: HOME-based resolution; no OPERATOR_DATA_DIR override needed here.
func registryFile() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".operator", "windows-pty-hosts.json"), nil
}
