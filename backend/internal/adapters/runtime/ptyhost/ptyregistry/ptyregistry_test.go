...
func setupHome(t *testing.T) string {
	...
	return filepath.Join(dir, ".operator", "windows-pty-hosts.json")
}
...
func TestRegisterThenList(t *testing.T) {
	...
	e := Entry{SessionID: "s1", PtyHostPID: 1234, PipePath: `\\.\pipe\opr-s1`, RegisteredAt: nowRFC3339()}
...
func TestRegisterReplaceSameID(t *testing.T) {
	...
	e1 := Entry{SessionID: "s1", PtyHostPID: 111, PipePath: `\\.\pipe\opr-s1-a`, RegisteredAt: nowRFC3339()}
	e2 := Entry{SessionID: "s1", PtyHostPID: 222, PipePath: `\\.\pipe\opr-s1-b`, RegisteredAt: nowRFC3339()}
...
func TestListPrunesDeadPIDs(t *testing.T) {
	...
	e1 := Entry{SessionID: "s1", PtyHostPID: 1, PipePath: `\\.\pipe\opr-s1`, RegisteredAt: nowRFC3339()}
	e2 := Entry{SessionID: "s2", PtyHostPID: 2, PipePath: `\\.\pipe\opr-s2`, RegisteredAt: nowRFC3339()}
	if err := Register(e1); err != nil {
		t.Fatal(err)
	}
	if err := Register(e2); err != nil {
...
func TestEmptyResultDeletesFile(t *testing.T) {
	...
	e := Entry{SessionID: "s1", PtyHostPID: 1, PipePath: `\\.\pipe\opr-s1`, RegisteredAt: nowRFC3339()}
	if err := Register(e); err != nil {
		t.Fatal(err)
	}
...
func TestAtomicWriteProducesValidJSON(t *testing.T) {
	...
	e := Entry{SessionID: "s1", PtyHostPID: 99, PipePath: `\\.\pipe\opr-s1`, RegisteredAt: nowRFC3339()}
	if err := Register(e); err != nil {
		t.Fatal(err)
	}
