...
// its exact Operator supervisor. After that supervisor exits and leaves the
...
// IsExactSupervisedProcessAlive reports only the Operator supervisor matching ref
...
func (r *Runtime) SendMessage(ctx context.Context, handle ports.RuntimeHandle, message string) error {
	...
	if message != "" {
		...
		// caller; they are not retried because Operator cannot safely distinguish
...
// (see tmux's main()); Operator's daemon is typically started without an
...
// difference for the still-correct box-drawing case. Operator already treats the
...
func (r *Runtime) attachCommand(handle ports.RuntimeHandle) ([]string, error) {
	...
	// xterm-256color palette. -T is available in Operator's minimum tmux version (3.2).
...
func buildLaunchCommand(cfg ports.RuntimeConfig) string {
	...
	// The Operator web terminal and tmux attach client both support 24-bit SGR color.
	...
	if cfg.Env["OPERATOR_SUPERVISED_PROCESS"] == "1" {
