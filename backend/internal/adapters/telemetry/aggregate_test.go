...
func TestAggregatingSinkPassesThroughUnaggregatedNamesImmediately(t *testing.T) {
	...
	s := NewAggregatingSink(rec, []string{"opr.http.5xx"}, time.Hour)
	defer s.Close(context.Background())

	s.Emit(context.Background(), ports.TelemetryEvent{Name: "opr.cli.invoked"})

	events := rec.snapshot()
	if len(events) != 1 || events[0].Name != "opr.cli.invoked" {
		t.Fatalf("events = %#v, want one immediate opr.cli.invoked passthrough", events)
	}
}
...
func TestAggregatingSinkFoldsBurstIntoOneRollupOnFlush(t *testing.T) {
	...
	if events[0].Name != "opr.http.5xx" {
		t.Fatalf("rollup name = %q, want opr.http.5xx", events[0].Name)
	}
...
func TestAggregatingSinkTracksEventNamesIndependently(t *testing.T) {
	rec := &syncRecordingSink{}
	s := NewAggregatingSink(rec, []string{"opr.http.5xx", "opr.daemon.panic"}, time.Hour)

	for i := 0; i < 3; i++ {
		s.Emit(context.Background(), ports.TelemetryEvent{Name: "opr.http.5xx"})
	}
	s.Emit(context.Background(), ports.TelemetryEvent{Name: "opr.daemon.panic"})
	s.flush(context.Background())
	...
	if counts["opr.http.5xx"] != 3 {
		t.Fatalf("opr.http.5xx rollup count = %d, want 3", counts["opr.http.5xx"])
	}
	if counts["opr.daemon.panic"] != 1 {
		t.Fatalf("opr.daemon.panic rollup count = %d, want 1", counts["opr.daemon.panic"])
	}
}
