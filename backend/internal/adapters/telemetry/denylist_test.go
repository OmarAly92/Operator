...
func TestDenylistSinkPassesEverythingWhenUnconfigured(t *testing.T) {
	...
	emitAll(sink, "opr.app.active", "opr.session.spawned")
...
func TestDenylistSinkDropsNamedEvents(t *testing.T) {
	next := &recordingSink{}
	sink := NewDenylistSink(next, []string{"opr.session.spawned"})
	emitAll(sink, "opr.app.active", "opr.session.spawned", "opr.http.5xx")
	if len(next.events) != 2 || forwardedNames(next)[0] != "opr.app.active" || forwardedNames(next)[1] != "opr.http.5xx" {
...
func TestDenylistSinkMatchesEitherInternalOrExportedName(t *testing.T) {
	for _, denied := range []string{"opr.app.active", "opr.v2.app.active"} {
		next := &recordingSink{}
		sink := NewDenylistSink(next, []string{denied})
		emitAll(sink, "opr.app.active")
...
func TestDenylistSinkSupportsPrefixWildcardAndIsCaseInsensitive(t *testing.T) {
	next := &recordingSink{}
	sink := NewDenylistSink(next, []string{"  OPR.RENDERER.*  "})
	emitAll(sink, "opr.renderer.route_viewed", "opr.renderer.api_error", "opr.session.spawned")
	if len(next.events) != 1 || forwardedNames(next)[0] != "opr.session.spawned" {
