...
// Workspace creates per-session plain directories under Operator's managed root.
...
// ForceDestroy intentionally follows Destroy semantics for scratch. Normal Operator
...
func (w *Workspace) managedPath(cfg ports.WorkspaceConfig) (string, error) {
	...
	return w.validateManagedPath(filepath.Join(w.managedRoot, string(cfg.ProjectID), "workers", string(cfg.SessionID)))
}
