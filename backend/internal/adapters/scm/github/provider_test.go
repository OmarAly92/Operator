...
func TestAuthenticatedIdentityClassifiesBot(t *testing.T) {
	f := newFakeGH(t)
	f.on(http.MethodGet, "/user", func(w http.ResponseWriter, _ *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]any{"login": "opr-bot", "type": "Bot"})
	})
	p := newProviderForTest(t, f)
	...
	if got != (ports.SCMIdentity{Login: "opr-bot", Human: false}) {
...
// GitHub answered 304, leaving Operator stuck on "Checks running" until the five-minute
...
func TestCommitChecksGuard_OtherRunTransitionInvalidatesGuard(t *testing.T) {
	...
	// returned 304 and Operator kept "Checks running". With the complete-representation
