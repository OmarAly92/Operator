// Package crush adapts Crush as an experimental user-approved Operator reviewer.
package crush
...
import (
	...
	workercrush "github.com/OmarAly92/operator/backend/internal/adapters/agent/crush"
...
const HostTrustWarning = "experimental user-approved reviewer: Crush uses normal permission prompts; Operator deliberately does not enable --yolo"
...
func (r *Reviewer) ReviewCommand(ctx context.Context, inv ports.ReviewInvocation) (ports.ReviewCommandSpec, error) {
	...
	message := fmt.Sprintf("Read and follow the Operator reviewer role in `%s`, then %s", filepath.ToSlash(inv.SystemPromptFile), strings.TrimSpace(inv.Prompt))
