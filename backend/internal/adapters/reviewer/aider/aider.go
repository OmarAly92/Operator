// Package aider adapts Aider's normal interactive chat TUI for Operator reviews.
package aider

import (
	"context"

	workeraider "github.com/OmarAly92/operator/backend/internal/adapters/agent/aider"
...
func (r *Reviewer) ReviewCommand(ctx context.Context, inv ports.ReviewInvocation) (ports.ReviewCommandSpec, error) {
	...
	message := ""
	if inv.TaskPromptFile != "" {
		message = "Follow the Operator reviewer role and review task loaded as read-only context."
	}
...
// ReviewProcessReusable reports false because Aider cannot add future Operator-owned
