// Package kiro adapts Kiro's interactive terminal UI for Operator code reviews.
package kiro
...
import (
	...
	workeragent "github.com/OmarAly92/operator/backend/internal/adapters/agent/kiro"
	"github.com/OmarAly92/operator/backend/internal/domain"
	"github.com/OmarAly92/operator/backend/internal/ports"
)

const reviewerAgentName = "opr-reviewer"

// Reviewer is Kiro's Operator code-review adapter.
...
// avoids positional input and --no-interactive; Operator injects InitialMessage only
// after the pane exists. The process cwd is an Operator-owned empty directory, so
...
func writeReviewerAgent(reviewerDir string, inv ports.ReviewInvocation) error {
	...
	system, err := os.ReadFile(inv.SystemPromptFile) //nolint:gosec // Operator-owned prompt path
	if err != nil {
...
- Shell is deny-by-default. Only the narrow git inspection and review-reporting command shapes configured by Operator can run.
...
- For each JSON reporting body, base64-encode the UTF-8 JSON yourself and use the task's command with the JSON replaced by the base64 text: printf '%s' '<base64>' | base64 --decode | gh api ... or ... | opr review submit .... Use base64 -D instead of --decode on systems that require it.
...
`
	...
	config := map[string]any{
		...
		"description":    "Operator read-only code reviewer",
...
func shellAllowedCommands(inv ports.ReviewInvocation) []string {
	...
	return []string{
		...
		`^` + base64Pipe + `opr review submit --session ` + session + ` --reviews -$`,
	}
}
