// Package goose defines Operator's experimental host-trusted Goose reviewer.
package goose
...
import (
	...
	workergoose "github.com/OmarAly92/operator/backend/internal/adapters/agent/goose"
...
const (
	...
	gooseBinary       = "/opt/opr/bin/goose"
	gatewayMCPBinary  = "/opt/opr/bin/opr-review-gateway-mcp"
	containedRoot     = "/opt/opr/reviewer"
	...
	modelBrokerHost   = "http://opr-review-model-broker/v1"
...
// ReviewCommand launches Goose's normal interactive run mode with Operator-owned CLI
// state. The first review task is passed as Goose's native instructions file so
// Operator never has to type a backtick-wrapped task path into a pane that may have
...
func (r *Reviewer) ReviewCommand(ctx context.Context, inv ports.ReviewInvocation) (ports.ReviewCommandSpec, error) {
	...
	if err != nil {
		return ports.ReviewCommandSpec{}, fmt.Errorf("goose reviewer: prepare Operator-owned profile: %w", err)
	}
...
// ReviewMessage keeps subsequent reviews in the already-running TUI. Operator's
...
// active turn; Operator must not send a second interrupt that could exit the TUI.
...
func hostGooseEnvironment(profile reviewgateway.Environment) map[string]string {
	env := map[string]string{
		"OPERATOR_DATA_DIR": profile.DataDir,
