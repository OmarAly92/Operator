// Package grok adapts Grok Build as an experimental user-approved Operator reviewer.
package grok
...
import (
	...
	workergrok "github.com/OmarAly92/operator/backend/internal/adapters/agent/grok"
	"github.com/OmarAly92/operator/backend/internal/domain"
	"github.com/OmarAly92/operator/backend/internal/ports"
)

// HostTrustWarning documents that Grok relies on its normal interactive
// approval flow rather than an Operator-enforced read-only sandbox.
const HostTrustWarning = "experimental user-approved reviewer: Grok uses its normal permission prompts because Operator cannot enforce a read-only sandbox"
...
// ReviewCommand starts Grok with Operator's reviewer role as native rules. It emits
...
func (r *Reviewer) ReviewCommand(ctx context.Context, inv ports.ReviewInvocation) (ports.ReviewCommandSpec, error) {
	...
	rules, err := os.ReadFile(inv.SystemPromptFile) //nolint:gosec // Operator-owned prompt path
	if err != nil {
