// Package devin adapts Devin for Terminal as an experimental host-trusted Operator
...
import (
	...
	workerdevin "github.com/OmarAly92/operator/backend/internal/adapters/agent/devin"
	"github.com/OmarAly92/operator/backend/internal/domain"
	"github.com/OmarAly92/operator/backend/internal/ports"
)

// HostTrustWarning describes the authority retained by Devin's interactive
// terminal. Operator deliberately does not add Devin's --sandbox flag.
...
func initialMessage(inv ports.ReviewInvocation) (string, error) {
	...
	return fmt.Sprintf(
		"Read and follow the Operator reviewer role in `%s`, then %s",
