...
const workspace = mustEnv("OPERATOR_PI_REVIEW_WORKSPACE");
const promptRoot = mustEnv("OPERATOR_PI_REVIEW_PROMPT_ROOT");
const workerSession = mustValue("OPERATOR_PI_REVIEW_SESSION");
const manifestPointer = mustEnv("OPERATOR_PI_REVIEW_MANIFEST_POINTER");
...
async function allowedReadPath(input: string): Promise<string> {
	...
	throw new Error("read path is outside the checkout and Operator prompt directory");
}
...
async function activeManifest(): Promise<ReviewManifest> {
	...
	if (pointerPath !== manifestPointer) throw new Error("active manifest pointer must be a regular Operator-owned path");
	...
	if (!pathWithin(manifestRoot, manifestPath)) throw new Error("active manifest is outside the Operator manifest directory");
...
async function authorizedTask(runId: string, prUrl: string, targetSha: string): Promise<ReviewTask> {
	...
	if (!task || task.prUrl !== prUrl || task.targetSha !== targetSha) {
		throw new Error("run, pull request, and target commit are not an exact active Operator review task");
	}
	return task;
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: "Post one COMMENT review, including optional inline findings, to the exact run, pull request, and target commit authorized by Operator's active manifest. Returns the GitHub review id.",
	...
	pi.registerTool({
		...
		async execute(_id, params, signal) {
			...
			try {
				const manifest = await activeManifest();
				for (const review of params.reviews) if (!manifest.tasks.some((task) => task.runId === review.runId)) throw new Error(`run ${review.runId} is not in the active Operator review manifest`);
				dir = await mkdtemp(join(promptRoot, "opr-submit-"));
				const input = join(dir, "reviews.json");
				await writeFile(input, JSON.stringify({ reviews: params.reviews }), { mode: 0o600 });
				return await run(pi, "opr", ["review", "submit", "--session", workerSession, "--reviews", input], signal);
			} catch (error) { return result(String(error), true); }
			finally { if (dir) await rm(dir, { recursive: true, force: true }); }
		},
	});
