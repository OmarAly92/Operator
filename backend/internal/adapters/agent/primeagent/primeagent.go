...
// GetConfigSpec reports the per-project Prime Agent configuration Operator exposes.
...
// so Operator intentionally emits no permission arguments for any requested mode.
...
func resolveSystemPrompt(ctx context.Context, inline, file string) (string, error) {
	...
	data, err := os.ReadFile(file) //nolint:gosec // path comes from Operator-owned launch configuration
...
// GetRestoreCommand intentionally reports native restore as unavailable. Operator
// launches with --no-session so Prime cannot leave a detached resident worker
// alive after Operator terminates the owning terminal session.
...
// safe pre/post correlation Operator requires for automated permission handling.
