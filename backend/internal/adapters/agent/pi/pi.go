...
// terminal coding harness. Operator runs Pi interactively in the session terminal
// pane. The initial prompt is delivered in-command as a trailing positional
// message; Pi's argument parser does not honor a `--` options terminator, so Operator
...
// confirmation flows are built via TypeScript extensions), so Operator emits no
...
// extensions (pi.on("session_start", ...), etc.), not a config file Operator can
...
func (p *Plugin) GetLaunchCommand(ctx context.Context, cfg ports.LaunchConfig) (cmd []string, err error) {
	...
	if cfg.SystemPrompt != "" {
		...
		data, err := os.ReadFile(cfg.SystemPromptFile) //nolint:gosec // path is Operator-owned launch config
		if err != nil {
			return nil, err
		}
