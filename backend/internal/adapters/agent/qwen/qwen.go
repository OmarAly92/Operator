...
// continue a specific session. Operator starts prompted worker sessions through Qwen's
...
// so Operator captures native session identity and activity from those hooks rather
...
// appendApprovalFlags maps Operator's four permission modes onto Qwen Code's
