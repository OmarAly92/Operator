...
const (
	...
	// qwenHookCommandPrefix identifies the hook commands Operator owns, so install
	// skips duplicates and uninstall recognizes Operator entries by prefix.
	qwenHookCommandPrefix = "opr hooks qwen "
...
// qwenManagedHooks is the source of truth for the hooks Operator installs:
...
// qwenHooks manages Operator's hooks in the workspace-local .qwen/settings.json file.
...
// GetAgentHooks installs Operator's Qwen Code hooks, preserving user-defined hooks and unrelated settings.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	return qwenHooks.Install(ctx, cfg.WorkspacePath)
}

// UninstallHooks removes Operator's Qwen Code hooks, leaving user-defined hooks untouched.
func (p *Plugin) UninstallHooks(ctx context.Context, workspacePath string) error {
	return qwenHooks.Uninstall(ctx, workspacePath)
}

// AreHooksInstalled reports whether any Operator Qwen Code hook is present.
