...
// `--print` (alias `-p`) which runs a single instruction and exits -- the mode Operator
...
// as an Operator-owned file, is injected via Auggie's `--rules` flag, which appends
...
// Because there is no verifiable blanket auto-approve flag, every Operator permission
...
// `--print` for headless resume. Operator only has a native session id to resume from
...
// system-prompt surface; Operator standing instructions use `--rules` when the
...
// Auggie's error; Operator does not silently retry with Auggie's default model.
...
// Operator emits no approval flag and defers every mode to the user's Auggie config.
