...
func TestLaunchAndRestoreRegisterOperatorMCPServer(t *testing.T) {
	...
	want := any(map[string]any{"mcpServers": map[string]any{"operator": map[string]any{"command": "/opt/operator/opr", "args": []any{"mcp"}, "env": map[string]any{"OPERATOR_SESSION_ID": "opr-1"}}}})
