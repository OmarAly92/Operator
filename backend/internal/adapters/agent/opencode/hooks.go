...
const (
	...
	// "{plugin,plugins}/*.{ts,js}"). Operator writes the plural `plugins/`, matching
	...
	// opencodePluginFileName is the Operator-owned plugin file. Operator fully owns this
	...
	opencodePluginFileName = "opr-activity.ts"

	// opencodePluginSentinel marks the file as Operator-managed. AreHooksInstalled and
	...
	opencodePluginSentinel = "operator: managed opencode activity plugin"

	// opencodeHookCommandPrefix identifies the hook commands Operator owns. The
	// embedded plugin shells `opr hooks opencode <event>`; this prefix is the
...
// structure Operator assembles (the way it builds Codex/Claude hook JSON).
//
...
// assert the plugin wires every one via the `opr hooks opencode <event>` command.
...
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	if _, err := os.Stat(pluginPath); err == nil {
		...
		if !managed {
			return fmt.Errorf("opencode.GetAgentHooks: refusing to overwrite non-Operator file at %s — move it so Operator can install its plugin", pluginPath)
		}
...
// AreHooksInstalled reports whether Operator's opencode plugin is present in the
...
func (p *Plugin) AreHooksInstalled(ctx context.Context, workspacePath string) (bool, error) {
	...
	managed, err := isOperatorManagedPlugin(opencodePluginPath(workspacePath))
	if err != nil {
