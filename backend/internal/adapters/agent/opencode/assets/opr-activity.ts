// operator: managed opencode activity plugin (do not edit)
//
// It maps opencode's native lifecycle events onto Operator's three normalized
// activity events:
//   session.created                       -> `opr hooks opencode session-start`
//   message.updated / message.part.updated -> `opr hooks opencode user-prompt-submit`
//   session.status (status.type == idle)   -> `opr hooks opencode stop`
//
// The opencode-native session id (and prompt/model where known) is piped to the
// hook command as JSON on stdin, run with cwd set to the worktree so Operator can
// correlate the opencode session to its Operator session. Every invocation is
// best-effort and must never crash the user's opencode session: a missing `opr`
// binary is a guarded no-op (`command -v opr`), and spawn exceptions, non-zero
...
export const operatorActivity: Plugin = async ({ directory, client }) => {
  // opr hooks must never be able to hang opencode: cap each invocation, matching
  ...
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v opr >/dev/null 2>&1; then exit 0; fi; exec opr hooks opencode ${hookName}`]
  }
