...
// Codex (0.136+) never loads hook config from Operator's per-session worktrees, so
// Operator's hooks ride the launch command as `-c` session-flag config instead of
// workspace files:
//
...
//     hooks.json written into an Operator worktree is therefore dead config.
...
const (
	...
	// codexHookCommandPrefix identifies the hook commands Operator owns, so the
	// legacy-file cleanup and uninstall recognize Operator entries by prefix
	// without an embedded template to diff against.
	codexHookCommandPrefix = "opr hooks codex "
	// codexHookTimeout caps how long Codex waits on one Operator hook callback. The
...
// codexHookSpec describes one hook Operator delivers via launch-command config.
type codexHookSpec struct {
	Event   string
	Command string
}

// codexManagedHooks is the source of truth for the hooks Operator delivers. Event
...
// appendSessionHookFlags adds Operator's activity hooks to the argv as `-c`
...
// this exact Operator executable instead of relying on a bare `opr` lookup.
func appendSessionHookFlags(cmd *[]string) error {
	executable, err := os.Executable()
	if err != nil {
		return fmt.Errorf("resolve Operator hook executable: %w", err)
	}
	if !filepath.IsAbs(executable) {
		executable, err = filepath.Abs(executable)
		if err != nil {
			return fmt.Errorf("make Operator hook executable absolute: %w", err)
		}
	}
...
// from Operator's worktrees (see the package comment above); activity hooks ride the
// launch command instead. It strips hook entries that older Operator versions wrote
// into the worktree-local .codex/hooks.json so reused or restored worktrees
// don't keep dead Operator config, preserving user-defined hooks.
...
// UninstallHooks removes Operator's legacy Codex hooks from the workspace-local
...
// removeLegacyWorkspaceHooks strips Operator-owned entries from a workspace-local
// hooks.json left behind by older Operator versions. Files without one are untouched.
...
// AreHooksInstalled reports whether any legacy Operator Codex hook is still present
...
// can tell whether stripping Operator entries changed anything, including removals
...
// removeCodexManagedHooks strips Operator hook entries from every group,
