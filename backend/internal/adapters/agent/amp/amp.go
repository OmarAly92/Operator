...
//
// Operator injects standing session instructions through a workspace-local Amp
...
// chooses the underlying models for a mode, so Operator exposes mode rather than a
...
// PromptReadinessHints waits briefly for Amp's interactive prompt before Operator
