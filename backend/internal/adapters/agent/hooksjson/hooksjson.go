...
// adapters differed only in the file path, the Operator command prefix, the per-hook
...
// Operator does not own, and writes atomically, so installing Operator's hooks never clobbers
// unrelated settings.
package hooksjson
...
// UnmarshalJSON retains fields introduced by an agent before Operator learns about
// them. User hooks may depend on agent-specific keys such as async or
// commandWindows, and reconciling Operator hooks must not remove them.
...
// HookEntry: reconciling one Operator hook must not rewrite unrelated user behavior.
...
// HookSpec describes one hook Operator installs: the native event it attaches to, its
...
// Manager installs, removes, and detects Operator's hooks in one agent's matcher-group
...
type Manager struct {
	...
	Label string
	// CommandPrefix identifies Operator-owned hook commands, e.g. "opr hooks goose ".
	...
	// Managed is the set of hooks Operator installs.
	Managed []HookSpec
}

// Install reconciles Operator's managed hooks into the workspace's hooks file,
...
// Uninstall removes Operator's hooks from the workspace's hooks file, leaving
...
// AreInstalled reports whether any Operator hook is present in the workspace's hooks
...
// sub-map, preserving every key Operator doesn't manage. A missing or empty file
// yields empty maps.
...
// reconcileHook removes every copy of one Operator-owned command before adding the
...
func reconcileHook(groups []MatcherGroup, hook HookEntry, matcher *string) []MatcherGroup {
	...
	for _, group := range groups {
		...
		if len(kept) > 0 {
			...
			// in-place refresh of Operator's hook.
			group.Hooks = nil
			result = append(result, group)
...
// removeManaged strips Operator hook entries (matched by command prefix) from every
