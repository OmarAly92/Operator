...
const (
	...
	hookCommandPrefix = "opr hooks kimchi "
	hookTimeout       = 30
)
...
// managedHooks is the source of truth for the hooks Operator installs into
...
// The Operator sub-command post-tool-use-failure is used (not post-tool-use-fail)
...
// kimchiHooks manages Operator's hooks in the workspace-local
...
// GetAgentHooks installs Operator's Kimchi hooks, preserving user-defined hooks and
// unrelated settings.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	return kimchiHooks.Install(ctx, cfg.WorkspacePath)
}

// UninstallHooks removes Operator's Kimchi hooks, leaving user-defined hooks untouched.
func (p *Plugin) UninstallHooks(ctx context.Context, workspacePath string) error {
	return kimchiHooks.Uninstall(ctx, workspacePath)
}

// AreHooksInstalled reports whether any Operator Kimchi hook is present.
