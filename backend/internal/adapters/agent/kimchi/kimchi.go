...
// @earendil-works/pi-coding-agent. Operator drives it non-interactively with
...
// permission modes. Operator maps its own modes onto these flags.
//
// Restore: Kimchi persists sessions and resumes by id with `--session <id>`.
// The native session id is captured from hook metadata and stored in Operator's
...
// Operator installs hooks in that file using `opr hooks kimchi <event>` commands.
...
// implementation is needed; Operator worktree spawns will not hang on a trust dialog.
//
...
// would require Operator to use pi.sendMessage directly instead of terminal input.
package kimchi
...
func readBoundedSystemPrompt(ctx context.Context, path string) (string, error) {
	...
	f, err := os.Open(path) //nolint:gosec // path is Operator-owned config
	if err != nil {
		return "", fmt.Errorf("kimchi: read system prompt file: %w", err)
	}
...
// --auto and --plan; only --dangerously-skip-permissions (not mapped by Operator)
