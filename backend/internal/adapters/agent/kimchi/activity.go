...
// onto an Operator activity state. Kimchi's native hook-adapter system emits
// events with the same names and payload formats that Operator installs. The
...
func sessionEndState(payload []byte) (domain.ActivityState, bool) {
	...
	case "reload", "new", "resume", "fork":
		...
		// Operator session stays alive. Report nothing so the activity feed is
