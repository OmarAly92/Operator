...
// token in `opr hooks <agent> <event>` onto the function that interprets that
// agent's hook callbacks as an Operator activity state.
//
// The hidden `opr hooks` CLI command dispatches a live callback through it. Every
// adapter that installs `opr hooks <tok>` callbacks must have a deriver
...
import (
	...
	"github.com/OmarAly92/operator/backend/internal/adapters/agent/fake"
...
// DeriveFunc maps a native agent hook event and its raw stdin payload onto an Operator
...
// Derivers maps the agent token in `opr hooks <agent> <event>` to its deriver.
...
// a registered deriver here means its adapter installs `opr hooks <harness>`
...
// harness can ever report (idle). Harness names and `opr hooks` agent tokens are
