...
const (
	...
	// .kiro/agents/<name>.json. Operator installs its hooks into a dedicated agent
	...
	kiroAgentFileName = "opr.json"

	// kiroHookCommandPrefix identifies the hook commands Operator owns, so install
	// skips duplicates and uninstall recognizes Operator entries by prefix without an
	// embedded template to diff against.
	kiroHookCommandPrefix = "opr hooks kiro "

	kiroAgentName        = "opr"
	kiroAgentDescription = "Operator session instructions"
)

// kiroHookFile is the on-disk shape of .kiro/agents/opr.json. It is used by
...
// kiroHookSpec describes one hook Operator installs, defined in code rather than read
// from an embedded hooks file.
type kiroHookSpec struct {
	// Event is the native Kiro hook event name (camelCase).
	Event string
	// Command is the Operator hook command line.
	Command string
}

// kiroManagedHooks is the source of truth for the hooks Operator installs. The native
// Kiro events are mapped onto Operator hook sub-command names (the trailing word) so
...
// GetAgentHooks installs Operator's Kiro hooks into the worktree-local
// .kiro/agents/opr.json file. Existing hook entries are preserved and duplicate
...
// UninstallHooks removes Operator's Kiro hooks from the workspace-local
// .kiro/agents/opr.json file, leaving user-defined hooks untouched. A missing
// file is a no-op.
...
// AreHooksInstalled reports whether any Operator Kiro hook is present in the
...
// "hooks" sub-map, preserving keys Operator doesn't manage. A missing or empty file
// yields empty maps.
...
// kiroManagedEvents returns the distinct Kiro events Operator manages, in the order
...
// removeKiroManagedHooks strips Operator hook entries from an event's array.
