...
// Operator injects prompted tasks after startup. Its non-interactive
// `goose run -t "<text>"` mode exits after the prompt completes, which is not a
// usable lifecycle for Operator worker terminals. Goose has a native
...
// etc. Operator installs its hooks there, so Operator derives native session identity and
...
//
// Note: the Operator repo also vendors pressly/goose as its SQLite migration tool,
...
// one of --instructions, --text, or --recipe, so Operator supplies empty text plus
...
// GetPromptDeliveryStrategy reports that Operator should inject prompted Goose tasks
...
// callers can fall back to fresh launch behavior. Operator deliberately uses run
...
// the model within the configured provider, which matches Operator's single-string
...
// gooseMode maps an Operator permission mode onto Goose's GOOSE_MODE value.
//
