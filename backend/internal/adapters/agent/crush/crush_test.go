...
func TestGetLaunchCommandBuildsCrossPlatformArgv(t *testing.T) {
	...
	// cfg.SessionID is the Operator-internal id and must NOT be passed as --session on
...
func TestGetAgentHooksInstallsSystemPromptContext(t *testing.T) {
	...
	if err := os.WriteFile(promptFile, []byte("Operator standing instructions\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	text := string(data)
	for _, want := range []string{crushSystemPromptMarker, "Operator standing instructions"} {
...
func TestGetAgentHooksMergesExistingCrushConfig(t *testing.T) {
	...
	for range 2 {
		if err := (&Plugin{}).GetAgentHooks(context.Background(), ports.WorkspaceHookConfig{
			WorkspacePath: workspace,
			SystemPrompt:  "Operator standing instructions",
		}); err != nil {
			t.Fatalf("GetAgentHooks err = %v", err)
		}
	}
	...
	if countJSONStrings(paths, crushSystemPromptPath) != 1 {
		t.Fatalf("context_paths duplicated Operator path: %#v", paths)
	}
}
...
func TestGetAgentHooksGitignoresManagedCrushFiles(t *testing.T) {
	...
	// Operator must not write/own a repo-root .gitignore: EnsureWorkspaceGitignore
	// is a no-op when a root .gitignore already exists without Operator's
...
func TestGetAgentHooksRefusesForeignSystemPromptFile(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "refusing to overwrite non-Operator file") {
		t.Fatalf("err = %v", err)
	}
}
