...
//
// Crush differs from other agents in that it doesn't yet expose Operator-compatible
// activity hooks. GetAgentHooks only injects Operator's standing system prompt through
...
const (
	...
	// `opr spawn --agent`. It matches domain.HarnessCrush.
	adapterID = "crush"
)
...
// model id per selection (see mergeCrushModel in hooks.go), so Operator's plain
...
// have a launch-time system-prompt flag, so GetAgentHooks installs Operator's system
// prompt as a workspace-local context file before launch. Worker task prompts
// are delivered after startup so Operator keeps the interactive TUI; Crush's
...
// Operator-internal id, not a Crush-native session id. Letting Crush mint its own
...
// PromptReadinessHints waits for Crush's ready prompt before Operator injects the
// worker's first task.
