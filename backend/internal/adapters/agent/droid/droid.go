...
// no Claude Code compatibility layer, so Operator installs its own hooks into the
...
// interactive --model flag either, so Operator's graduated permission modes and any
...
// droidAutonomyLevel maps an Operator permission mode onto Droid's
...
func runtimeSettingsArgs(dataDir, sessionID string, mode ports.PermissionMode, model string) ([]string, error) {
	...
	if strings.TrimSpace(dataDir) == "" {
		return nil, fmt.Errorf("droid: Operator data directory required for runtime settings")
	}
...
// ~/.operator / OPERATOR_DATA_DIR). A stable name keyed by session id means relaunches
...
func runtimeSettingsPath(dataDir, sessionID string) string {
	...
	return filepath.Join(dataDir, "agent-runtime", "droid", "opr-droid-"+name+"-settings.json")
}
