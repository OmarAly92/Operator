...
// onto an Operator activity state. The bool is false when the event carries no
// activity signal — e.g. SessionStart (metadata only) or a SessionEnd reason
// that doesn't actually end the Operator session — in which case the caller reports
// nothing.
//
// event is the Operator hook sub-command name installed in droidManagedHooks
// ("user-prompt-submit", "stop", "notification", "session-end", ...), NOT the
// native Droid event name. Keeping this beside hooks.go means the events Operator
...
// permission decision or has been idle awaiting input for 60s. Operator cannot tell
...
// "clear" keeps the same Operator session alive (a new native session continues in
