...
const (
	...
	// droidHookCommandPrefix identifies the hook commands Operator owns, so install
	// skips duplicates and uninstall recognizes Operator entries by prefix.
	droidHookCommandPrefix = "opr hooks droid "
	droidHookTimeout       = 30
)
...
// droidManagedHooks is the source of truth for the hooks Operator installs:
...
// droidHooks manages Operator's hooks in the workspace-local .factory/hooks.json file.
...
// GetAgentHooks installs Operator's Droid hooks, preserving user-defined hooks.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	return droidHooks.Install(ctx, cfg.WorkspacePath)
}

// UninstallHooks removes Operator's Droid hooks, leaving user-defined hooks untouched.
func (p *Plugin) UninstallHooks(ctx context.Context, workspacePath string) error {
	return droidHooks.Uninstall(ctx, workspacePath)
}

// AreHooksInstalled reports whether any Operator Droid hook is present.
