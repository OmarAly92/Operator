...
// agent. Its binary is "cn" (npm package @continuedev/cli) and the Operator harness /
...
// callbacks through the existing "opr hooks claude-code <evt>" dispatcher — no
...
// adapterID is the Operator harness / manifest id. It is the string "continue"
...
//
// Operator sessions are long-lived terminal sessions, so prompted and promptless
// launches both stay interactive as `cn ...`. Permission flags map Operator's 4 modes
...
//
// The installed commands are "opr hooks claude-code <evt>", so the existing CLI
...
// (SessionStart / UserPromptSubmit / Stop / Notification), giving Operator
...
// appendApprovalFlags maps Operator's 4 permission modes onto Continue's two boolean
