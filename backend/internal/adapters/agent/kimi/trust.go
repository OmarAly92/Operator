...
// after-start prompt paste and blocks the readiness marker, so Operator seeds Kimi's
...
// folder. Operator writes the same shape so Kimi cannot distinguish seeded trust
