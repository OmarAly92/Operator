...
const (
	...
	museHookCommandPrefix  = "opr hooks muse "
	operatorRunFileEnvVar  = "OPERATOR_RUN_FILE"
)
...
func museManagedHooks(cfg ports.WorkspaceHookConfig) map[string][]hooksjson.MatcherGroup {
	...
	// otherwise idle Operator session. Track the top-level turn lifecycle instead;
...
// Muse sanitizes OPERATOR_* variables from hook subprocesses. Put the non-sensitive
// callback route directly in Operator's per-session hook command so the callback can
// identify its Operator session and, in dev mode, the exact daemon that launched it.
func museHookCommand(cfg ports.WorkspaceHookConfig, event string) string {
	assignments := []string{
		"OPERATOR_SESSION_ID=" + museShellQuote(strings.TrimSpace(cfg.SessionID)),
		"OPERATOR_DATA_DIR=" + museShellQuote(strings.TrimSpace(cfg.DataDir)),
	}
	if runFile := strings.TrimSpace(os.Getenv(operatorRunFileEnvVar)); runFile != "" {
		assignments = append(assignments, operatorRunFileEnvVar+"="+museShellQuote(runFile))
	}
...
// GetAgentHooks writes Muse's managed-hook configuration under Operator's data
...
// CleanupWorkspace removes the Operator-owned managed-hook file. The method name is
