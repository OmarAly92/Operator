...
const (
	...
	// autohandHookCommandPrefix identifies the hook commands Operator owns, so
	// install skips duplicates and uninstall recognizes Operator entries by prefix
	// without an embedded template to diff against.
	autohandHookCommandPrefix = "opr hooks autohand "
	autohandHookTimeout       = 30
)

// autohandManagedHookKeys are the entry keys Operator owns. On marshal they are
...
// hooks.hooks array. Operator owns the five typed fields; any other key the user set
// on an entry (matcher, filter, async, ...) is captured in Extra so a rewrite
// preserves fields Operator does not own instead of silently dropping them.
type autohandHookEntry struct {
	...
	// Extra holds keys Operator does not manage, captured on unmarshal and written
...
// UnmarshalJSON decodes the entry's typed fields and captures every key Operator does
...
// MarshalJSON writes Operator's managed fields merged with any preserved unknown keys
// from Extra. Managed fields win on key collision so Operator's values stay
// authoritative.
...
// autohandHookSpec describes one hook Operator installs. Event is Autohand's native
// lifecycle event name; Subcommand is the Operator hook sub-command appended after the
...
// autohandManagedHooks is the source of truth for the hooks Operator installs. Each
// native Autohand event is routed to the Operator sub-command DeriveActivityState
...
// GetAgentHooks installs Operator's Autohand hooks into the Autohand config's
// hooks.hooks array. Existing user hooks are preserved and duplicate Operator commands
...
// runtime, so Operator installs into that config rather than a per-workspace file. The
// AUTOHAND_CONFIG env var, when set, takes precedence so Operator and the agent agree
// on the target.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	for _, spec := range autohandManagedHooks {
		...
		entries = append(entries, autohandHookEntry{
			Event:       spec.Event,
			Command:     command,
			Description: "Operator activity hook",
			Enabled:     true,
...
// UninstallHooks removes Operator's Autohand hooks from the config's hooks.hooks
...
// enables every Autohand hook, not just Operator's.
...
// AreHooksInstalled reports whether any Operator Autohand hook is present in the
...
// "hooks" section (preserving keys Operator doesn't manage such as "enabled"), and the
