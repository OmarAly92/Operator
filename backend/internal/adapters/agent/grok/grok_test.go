...
func TestGetLaunchCommand(t *testing.T) {
	...
	cmd, err := plugin.GetLaunchCommand(context.Background(), ports.LaunchConfig{
		Prompt:       "do the thing",
		SystemPrompt: "opr standing instructions",
		Permissions:  ports.PermissionModeBypassPermissions,
	})
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	wantPrefix := []string{"grok", "--no-auto-update", "--permission-mode", "bypassPermissions", "--rules", "opr standing instructions"}
...
func TestGetRestoreCommand(t *testing.T) {
	...
	cmd, ok, err := plugin.GetRestoreCommand(context.Background(), ports.RestoreConfig{
		...
		SystemPrompt: "opr restore instructions",
		Permissions:  ports.PermissionModeBypassPermissions,
	})
	if err != nil {
		t.Fatalf("err: %v", err)
	...
	want := []string{"grok", "--no-auto-update", "--permission-mode", "bypassPermissions", "--rules", "opr restore instructions", "-r", "sess-abc123"}
	if !reflect.DeepEqual(cmd, want) {
		t.Fatalf("cmd = %#v, want %#v", cmd, want)
	}
	if strings.Contains(strings.Join(cmd, " "), "system-prompt-override") {
...
func TestGetRestoreCommandNoID(t *testing.T) {
	tests := []struct {
		...
		{"opr session id only", ports.SessionRef{ID: "opr-7"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			plugin := &Plugin{resolvedBinary: "grok"}
...
func TestGetAgentHooksInstallsGrokCommandsInClaudeSettings(t *testing.T) {
	...
	existing := `{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"my own stop hook","timeout":5},{"type":"command","command":"opr hooks claude-code stop","timeout":30}]}]},"permissions":{"defaultMode":"plan"}}`
	...
	for _, spec := range grokManagedHooks {
		...
		claudeCommand := strings.Replace(spec.Command, "opr hooks grok ", "opr hooks claude-code ", 1)
	...
	if countGrokHookCommand(config.Hooks["Stop"], "opr hooks claude-code stop") != 1 {
		t.Fatalf("existing Claude Operator Stop hook not preserved: %#v", config.Hooks["Stop"])
	}
	if len(config.Permissions) == 0 {
		t.Fatalf("unrelated settings clobbered: %s", data)
	}
	if m := grokMatcherForCommand(config.Hooks["SessionStart"], "opr hooks grok session-start"); m == nil || *m != "startup" {
		t.Fatalf("SessionStart matcher = %v, want startup", m)
	}
	if m := grokMatcherForCommand(config.Hooks["UserPromptSubmit"], "opr hooks grok user-prompt-submit"); m != nil {
		t.Fatalf("UserPromptSubmit matcher = %v, want none", m)
	}
	if m := grokMatcherForCommand(config.Hooks["Notification"], "opr hooks grok notification"); m != nil {
	...
	if countGrokHookCommand(config.Hooks["Stop"], "opr hooks claude-code stop") != 1 {
		t.Fatalf("Claude Operator Stop hook not preserved after uninstall: %#v", config.Hooks["Stop"])
	}
	if len(config.Permissions) == 0 {
