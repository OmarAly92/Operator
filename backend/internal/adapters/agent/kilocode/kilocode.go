...
// plugin runtime, so Operator bridges it the same two ways it bridges opencode:
...
//     so GetAgentHooks installs an Operator-owned plugin file (see hooks.go) into
//     .kilocode/plugins/ instead of merging JSON.
//   - Its interactive TUI exposes no permission flag (the --auto flag lives only
//     on `kilo run`, not the default TUI command Operator launches) and no
//     system-prompt flag. Operator's graduated permission modes and standing
...
const (
	...
	// `opr spawn --agent`. It matches domain.HarnessKilocode.
	adapterID = "kilocode"
)
...
// Shape:
//
//	[env KILO_CONFIG_CONTENT=<json>] kilocode [--agent <opr-agent>] [--prompt <prompt>]
//
// The session runs in the worktree (cwd is set by the runtime, as for opencode
// and Codex). Kilo Code has no CLI flag to set a system prompt, so Operator injects a
...
// session: `[env KILO_CONFIG_CONTENT=<json>] kilocode [--agent <opr-agent>] --session <agentSessionId>`.
// It re-applies the permission env and per-session Operator agent prompt (resume
...
// `kilo run`, not on the default TUI command Operator launches, so passing any
...
// kilocodePermissionConfig maps an Operator permission mode onto Kilo's permission
...
// empty when the command must select Operator's generated agent with --agent.
//
...
func kilocodeConfigEnvPrefix(mode ports.PermissionMode, inlinePrompt, promptFile, sessionID, model string) ([]string, string, error) {
	...
	// system prompt or a role-specific model must be carried on Operator's generated
	...
	if systemPrompt != "" || model != "" {
		agentName = kilocodeOperatorAgentName(sessionID)
...
func kilocodeOperatorAgentName(sessionID string) string {
	const fallback = "opr-system-prompt"
