// operator: managed kilocode activity plugin (do not edit)
//
...
// It maps Kilo's native lifecycle events onto Operator's normalized activity events:
//   session.created                        -> `opr hooks kilocode session-start`
//   message.updated / message.part.updated  -> `opr hooks kilocode user-prompt-submit`
//   permission.ask hook                     -> `opr hooks kilocode permission-request`
//   session.status (status.type == idle)    -> `opr hooks kilocode stop`
//
// The native session id (and prompt/model where known) is piped to the hook
// command as JSON on stdin, run with cwd set to the worktree so Operator can correlate
// the Kilo session to its Operator session. Every invocation is best-effort and must
// never crash the user's Kilo session: a missing `opr` binary is a guarded no-op
// (`command -v opr`), and spawn exceptions, non-zero exit codes, and malformed
...
export const operatorActivity: Plugin = async ({ directory, client }) => {
  // opr hooks must never be able to hang Kilo: cap each invocation, matching
  ...
  // Wrap in `sh -c` with a guard so a missing `opr` binary is a silent no-op
  // (exit 0) rather than a per-event error in the user's session.
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v opr >/dev/null 2>&1; then exit 0; fi; exec opr hooks kilocode ${hookName}`]
  }
  ...
  function logHookFailure(hookName: string, detail: string) {
    try {
      void client?.app
        ?.log?.({ body: { service: "opr-activity", level: "error", message: `hook ${hookName} failed: ${detail}` } })
        ?.catch?.(() => {})
    } catch {
  ...
  //      session-start, so Operator would see the prompt before the session is
  ...
  //
  // A non-zero exit (the guard makes a missing `opr` exit 0, so this is a real
  // `opr hooks` failure) or a spawn exception is logged with its stderr and never
  ...
  // prompt never reaches Operator and title-from-prompt metadata breaks. Therefore: an
  ...
  return {
    // permission.ask fires when Kilo needs the user to approve a tool call. Operator
      ...
      try {
        switch (event.type) {
          ...
          case "session.status": {
            ...
            // Operator's "stop" hook means "the current turn is idle/finished", not
