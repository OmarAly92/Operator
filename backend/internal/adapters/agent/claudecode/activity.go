...
// payload) onto an Operator activity state. The bool is false when the event carries
// no activity signal — e.g. SessionStart (metadata only, v1), a Notification
// type we don't track, or a SessionEnd reason that doesn't actually end the Operator
// session — in which case the caller reports nothing.
//
// event is the Operator hook sub-command name installed in claudeManagedHooks
// ("user-prompt-submit", "stop", "notification", "session-end", ...), NOT the
// native Claude event name. Keeping this beside hooks.go means the events Operator
...
// clear/resume keep the same Operator session alive (a new native session continues
