...
func TestEmbeddedSkillFrontmatterIsValidYAML(t *testing.T) {
	body, err := files.ReadFile("using-opr/SKILL.md")
	if err != nil {
...
func TestEmbeddedPreviewGuidanceDoesNotScaffoldStaticSites(t *testing.T) {
	previewBody, err := files.ReadFile("using-opr/commands/preview.md")
	if err != nil {
	...
	for _, required := range []string{
		...
		"Do not create `.operator/launch.json` unless the user asks",
	...
	skillBody, err := files.ReadFile("using-opr/SKILL.md")
	if err != nil {
		t.Fatalf("read embedded SKILL.md: %v", err)
	}
...
func TestEmbeddedBrowserGuidanceKeepsNetworkCaptureOptional(t *testing.T) {
	body, err := files.ReadFile("using-opr/commands/browser.md")
	if err != nil {
...
// skill (SKILL.md plus a commands file) under <dataDir>/skills/using-opr, and a
...
// materialize the skill into .opencode/skills/using-opr (not the data-dir layout).
