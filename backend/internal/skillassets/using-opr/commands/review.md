...
Manage Operator code reviews of a worker's PR.

## Syntax

```
opr review <subcommand> [args] [flags]
```
...
**Syntax:**
```
opr review submit [worker-session-id] [flags]
```
...
opr review submit mer-3 --run review-run-1 --verdict approved
```

```bash
# Submit a changes-requested review with a body from stdin
echo "Please fix the null check on line 42." | opr review submit --session mer-3 --run review-run-1 --verdict changes_requested --body -
```
