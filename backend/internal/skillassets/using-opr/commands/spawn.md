...
Spawn a worker agent session in a registered project. The session runs the chosen agent in a fresh git worktree. Register the project first with `opr project add`.
...
|---|---|---|
| `--branch string` | Branch for the session worktree | `opr/<session-id>/root` |
...
opr spawn --project operator --name "review-pr-88" --claim-pr 88 --harness claude-code
```
