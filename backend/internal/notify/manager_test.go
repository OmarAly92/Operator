...
func TestHubProjectFilter(t *testing.T) {
	...
	defer unsub()
	_ = hub.Publish(context.Background(), domain.NotificationEvent{Record: domain.NotificationRecord{ID: "skip", ProjectID: "opr"}})
...
// Resolution is how a notification leaves the unresolved list: Operator observed the
