...
var (
	...
	// How long a cached catalog is trusted before Operator asks a cache-first client to
...
func appendCacheWarning(current string) string {
	const next = "Models loaded, but Operator could not update the model cache."
	if current == "" {
		return next
	}
	return current + " " + next
