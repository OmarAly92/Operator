...
func (s *Service) Start(ctx context.Context, cfg StartConfig) (*Controller, error) {
	...
	if conversation.Scope == domain.ConversationScopeProject &&
		...
		if boundaryErr := s.store.UpsertActivity(ctx, conversation.ID, "", domain.ConversationActivity{
			...
			Summary:        "Started a fresh agent context. Earlier project history remains visible in Operator but was not loaded into this agent.",
			Detail:         detail,
			ProviderItemID: "opr-context-reset:" + string(cfg.SessionID),
	...
	if cfg.ProviderConversationID != "" {
		...
		//
		// Read Operator's existing projection too. ACP message/turn ids are opaque, and an
		// agent may assign a different persisted user id from the id Operator supplied at
...
// drivers can continue using Operator's model/settings surface.
...
//
// Read from the live conversation rather than a table in Operator: models are added,
...
// ConfigOptions reports the provider's live session controls. Unlike Operator's
...
// cannot go stale in an Operator table.
...
// RelayChatTurn delivers a message Operator is carrying for someone else.
//
// Origin is automation, not human: `opr send` and an orchestrator writing to a
// worker are Operator acting on the user's instructions, and the timeline attributes
