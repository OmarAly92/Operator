...
// provider's own record of the conversation, not Operator's view of it. Rollback is the
// sharp one — it changes what the agent REMEMBERS — so Operator's durable rows have to
...
var (
	...
	// no provider history to discard, so an undo would only hide Operator's own rows and
...
// race: Operator would have already decided to hide rows by the time it learned it could
// not. The controller holds its dispatch lock across the check and the call, so
// within Operator the answer cannot change underneath.
...
// new provider handle.
//
// Deliberately not reachable over HTTP yet, and the reason is not effort. Operator's
// schema allows one conversation per session, so a fork has to become a second Operator
...
//
// Nothing is written to Operator's rows here. The provider answers, then emits its own
// rename notification, and the projection applies it — so the title Operator stores is
...
// NormalizeTitle reduces a title to the one-line label Operator is willing to show.
//
...
// provider already accepted the name, and refusing to display a title Operator's own
