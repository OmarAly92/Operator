...
// Projection of the provider signal Operator used to drop: reasoning streams, plans,
...
func TestAccountReportsMergeRatherThanReplace(t *testing.T) {
	...
	if account.AuthMode != "chatgpt" || account.PlanLabel != "pro" {
		t.Fatalf("a credential demand blanked what Operator already knew: %+v", account)
	}
...
func TestAutoReviewIsItsOwnActivityKind(t *testing.T) {
	...
	snapshot := h.awaitSnapshot(t, func(s store.ConversationSnapshot) bool {
		return activityByItem(s, "opr-review-r1").Status == domain.ActivityStatusCompleted
	})
	row := activityByItem(snapshot, "opr-review-r1")
