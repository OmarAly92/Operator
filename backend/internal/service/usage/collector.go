...
// ErrUsageSessionNotFound reports that hook metadata targeted no durable Operator session.
...
// HookSignal is the usage-specific metadata carried by an Operator agent hook.
...
// ReactivateSession resumes collection for the native session relaunched by Operator.
...
// BackfillActive discovers transcript files only for live/resumable Operator
