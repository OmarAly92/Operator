...
// Operator-internal review passes.
...
// ReviewResult is the already-persisted result of an Operator-internal review pass.
...
func (m *Manager) ApplyReviewBatch(ctx context.Context, workerID domain.SessionID, batchID string, results []ReviewResult) (ReviewDeliveryOutcome, error) {
	...
	fmt.Fprintf(&msg, "[Operator reviewer] Operator's internal code reviewer submitted %d review(s) requesting changes.\n", len(results))
	var sigParts []string
...
//     for "assignee changed away from Operator" is reserved for the write-side work
//     tracked by #40.
...
func formatReviewChangesRequestedMessage(review domain.PullRequestReview) string {
	...
	msg.WriteString("\n\nAddress the requested changes and push. You should not need to re-fetch the review unless you need additional context beyond what Operator has provided here.")
	return msg.String()
}

func formatReviewCommentsMessage(comments []ports.PRCommentObservation) string {
	if len(comments) == 0 {
		return "A reviewer left feedback on your PR. Address it and push. Fetch the review details only if you need additional context beyond what Operator has provided here."
	}
	var msg strings.Builder
