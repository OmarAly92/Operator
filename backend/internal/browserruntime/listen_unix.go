...
var runtimeAliasPattern = regexp.MustCompile(`^opr-brd-(\d+)-[0-9a-f]{16}$`)
...
func createRuntimeAlias(root, runtimeDir string) (string, error) {
	for range 16 {
		...
		aliasPath := filepath.Join(root, fmt.Sprintf("opr-brd-%d-%s", os.Getpid(), hex.EncodeToString(random)))
