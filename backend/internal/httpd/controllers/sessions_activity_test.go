...
func TestSessionsAPI_ActivityForwardsUsageMetadataWithoutChangingActivity(t *testing.T) {
	...
	if usage.calls != 1 || usage.gotID != "opr-1" {
...
func TestSessionsAPI_ActivityAppliesSignal(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, "POST", "/api/v1/sessions/opr-1/activity", `{"state":"waiting_input"}`)
	if status != http.StatusOK {
		t.Fatalf("activity = %d, want 200; body=%s", status, body)
	}
	var resp struct {
	...
	if !resp.OK || resp.SessionID != "opr-1" || resp.State != "waiting_input" {
		t.Fatalf("activity response = %#v", resp)
	}
	if rec.calls != 1 || rec.gotID != "opr-1" {
...
func TestSessionsAPI_ActivityAcceptsBlocked(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, "POST", "/api/v1/sessions/opr-1/activity", `{"state":"blocked"}`)
	if status != http.StatusOK {
		t.Fatalf("activity = %d, want 200; body=%s", status, body)
	}
...
func TestSessionsAPI_ActivityRejectsUnknownState(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, "POST", "/api/v1/sessions/opr-1/activity", `{"state":"napping"}`)
...
func TestSessionsAPI_ActivityRejectsEmptyMetadataOnlyRequest(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, "POST", "/api/v1/sessions/opr-1/activity", `{"event":"session-start"}`)
...
func TestSessionsAPI_ActivityRecorderErrorIs500(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, "POST", "/api/v1/sessions/opr-1/activity", `{"state":"exited"}`)
...
func TestSessionsAPI_ActivityWithoutRecorderIs501(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, "POST", "/api/v1/sessions/opr-1/activity", `{"state":"idle"}`)
