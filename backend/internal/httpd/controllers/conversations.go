...
// real capability answer and keeps native drivers free to use Operator's older typed
...
// own event, and that report is what moves Operator's session label. Claiming 200 would
...
func writeConversationError(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, chatsvc.ErrTurnRunning):
		...
		// Operator's timeline and the agent's memory describing different conversations.
		envelope.WriteAPIError(w, r, http.StatusConflict, "conflict",
			"CHAT_TURN_RUNNING",
	...
	case errors.Is(err, ports.ErrChatResumeFailed):
		...
		// choice rather than have Operator invent a fresh conversation.
		envelope.WriteAPIError(w, r, http.StatusConflict, "conflict",
			"CHAT_RESUME_FAILED",
...
// into the settings payload: settings say what Operator asked for, and this says what
