...
func TestCLIInvokedRouteEmitsTelemetryForUserCommands(t *testing.T) {
	...
	if sink.events[1].Name != "opr.app.active" {
		t.Fatalf("second event name = %q, want opr.app.active", sink.events[1].Name)
	}
...
func TestCLIInvokedRouteDropsRoutineInternalSuccessTelemetry(t *testing.T) {
	...
	for _, body := range []string{
		`{"command":"status","commandPath":"opr status","actorType":"user"}`,
		`{"command":"ls","commandPath":"opr session ls","actorType":"user"}`,
		...
		`{"command":"hooks","commandPath":"OPR HOOKS","actorType":"user"}`,
		`{"command":"hooks","commandPath":"opr hooks claude-code post-tool-use","actorType":"user"}`,
		`{"command":"pty-host","commandPath":"opr pty-host","actorType":"system"}`,
	} {
		req := httptest.NewRequest(http.MethodPost, "http://127.0.0.1/internal/telemetry/cli-invoked", strings.NewReader(body))
		req.Host = "127.0.0.1:3001"
		req.Header.Set("Content-Type", "application/json")
...
func TestCLIInvokedRouteDropsUnknownLegacyCommandPaths(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "http://127.0.0.1/internal/telemetry/cli-invoked", strings.NewReader(`{"command":"surprise","commandPath":"opr surprise"}`))
	req.Host = "127.0.0.1:3001"
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
...
func TestCLIInvokedRouteSeparatesAgentAndSystemInvocationsFromActiveUsers(t *testing.T) {
	...
	// Older CLIs do not send actorType, so the daemon infers opr hooks as a
	// routine internal agent path and drops successful invocation telemetry.
	postInvoked(`{"command":"hooks","commandPath":"opr hooks"}`)
	if len(sink.events) != 0 {
	...
	postInvoked(`{"command":"pty-host","commandPath":"opr pty-host"}`)
	if len(sink.events) != 1 {
...
func TestCLIInvokedRouteDedupeIncludesActorType(t *testing.T) {
	...
	if sink.events[1].Name != "opr.cli.invoked" || sink.events[1].Payload["actor_type"] != "user" {
		t.Fatalf("second emitted event = %#v, want user opr.cli.invoked", sink.events[1])
	}
	if sink.events[2].Name != "opr.app.active" {
		t.Fatalf("third emitted event = %#v, want opr.app.active", sink.events[2])
	}
}
...
func TestCLIInvokedRouteRequiresLoopback(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "http://evil.example/internal/telemetry/cli-invoked", strings.NewReader(`{"command":"status","commandPath":"opr status"}`))
...
func TestCLIUsageErrorRouteEmitsTelemetry(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "http://127.0.0.1/internal/telemetry/cli-usage-error", strings.NewReader(`{"command":"status","commandPath":"opr status","error":"too many args"}`))
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
	...
	if len(sink.events) != 1 || sink.events[0].Name != "opr.cli.usage_errors" {
		t.Fatalf("events = %#v, want one opr.cli.usage_errors event", sink.events)
	}
	...
	if got := payload["command_path"]; got != "opr status" {
		t.Fatalf("payload.command_path = %#v, want opr status", got)
	}
...
func TestCLIUsageErrorRouteHashesInvalidCommandsBeforeRemoteExport(t *testing.T) {
	...
	req := httptest.NewRequest(
		...
		strings.NewReader(`{"command":"`+rawCommand+`","commandPath":"opr `+rawCommand+`","error":"too many args"}`),
	)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
	if rec.Code != http.StatusAccepted {
...
func TestRecoverTelemetryEmitsPanicEvent(t *testing.T) {
	...
	if panicPayload == nil {
		t.Fatalf("events = %#v, want opr.daemon.panic", sink.events)
	}
	if fiveXXPayload == nil {
		t.Fatalf("events = %#v, want opr.http.5xx after recovery", sink.events)
	}
