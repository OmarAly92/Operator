...
func TestAgentProcessSuperviseReportsExitAndPreservesOutput(t *testing.T) {
	...
	}, "agent-process", "supervise", "--session", "opr-7", "--launch", "launch-3", "--", "sh", "-c", "printf supervised; exit 23")
	if err != nil {
...
func TestAgentProcessSuperviseRejectsInvalidGeneration(t *testing.T) {
	_, _, err := executeCLI(t, Deps{}, "agent-process", "supervise", "--session", "opr-7", "--launch", "../stale", "--", "true")
	if err == nil {
