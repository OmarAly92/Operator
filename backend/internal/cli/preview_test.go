...
func previewLifecycleServer(t *testing.T, status int, respBody string) (*httptest.Server, *previewCapture) {
	t.Helper()
	t.Setenv("OPERATOR_BROWSER_CAPABILITY", "preview-capability")
	capture := &previewCapture{}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestPreview_WithURLArg(t *testing.T) {
	t.Setenv("OPERATOR_SESSION_ID", "aa-47")
	cfg := setConfigEnv(t)
...
func TestPreviewClear_MissingSessionIDIsUsageError(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected usage error when OPERATOR_SESSION_ID is unset")
	}
	if got := ExitCode(err); got != 2 {
		t.Fatalf("exit code = %d, want 2", got)
	}
	if capture.called {
		t.Fatal("daemon should not be contacted when OPERATOR_SESSION_ID is unset")
	}
}
...
func TestPreview_MissingSessionIDIsUsageError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "OPERATOR_SESSION_ID is not set") {
...
func TestPreview_BlankSessionIDIsUsageError(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected usage error when OPERATOR_SESSION_ID is blank")
	}
	if got := ExitCode(err); got != 2 {
		t.Fatalf("exit code = %d, want 2", got)
	}
	if capture.called {
		t.Fatal("daemon should not be contacted when OPERATOR_SESSION_ID is blank")
	}
}
