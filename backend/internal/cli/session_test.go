...
func TestSessionGetJSONIncludesBriefAndPRFields(t *testing.T) {
	cfg := setConfigEnv(t)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"session":{
	...
	out, _, err := executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "get", "opr-1", "--json")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got.Session.Brief != "fix the flaky resize test" {
		t.Fatalf("brief = %q", got.Session.Brief)
	}
	if got.Session.LatestUserPrompt != "also check the codex path" {
		t.Fatalf("latestUserPrompt = %q", got.Session.LatestUserPrompt)
	}
	if got.Session.LatestAssistantUpdate != "I reproduced it and pushed a fix." {
		t.Fatalf("latestAssistantUpdate = %q", got.Session.LatestAssistantUpdate)
	}
	if len(got.Session.PRs) != 1 || got.Session.PRs[0].Number != 7 || got.Session.PRs[0].State != "open" {
		t.Fatalf("prs = %+v", got.Session.PRs)
	}
}
...
func TestSessionClaimPR_ProjectScopeMismatchIsUsage(t *testing.T) {
	...
	_, _, err := executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "claim-pr", "demo-1", "https://github.com/OmarAly92/operator/pull/142", "-p", "other")
...
func TestSessionClaimPR_JSONAndNoTakeoverError(t *testing.T) {
	...
	out, errOut, err := executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "claim-pr", "demo-1", "https://github.com/OmarAly92/operator/pull/142", "--json")
	if err != nil {
	...
	_, _, err = executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "claim-pr", "demo-1", "https://github.com/OmarAly92/operator/pull/142", "--no-takeover")
...
func TestSessionClaimPR_GHFallbackWhenProjectRepoMissing(t *testing.T) {
	...
	out, _, err := executeCLI(t, Deps{
		...
		CommandOutputInDir: func(_ context.Context, dir, name string, args ...string) ([]byte, error) {
			...
			return []byte("https://github.com/OmarAly92/operator\n"), nil
		},
