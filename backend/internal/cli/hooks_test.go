...
func TestHooks_ReportsUsageTranscriptMetadata(t *testing.T) {
	...
	cfg := setConfigEnv(t)
	srv, capture := activityServer(t, http.StatusOK, `{"ok":true,"sessionId":"opr-7","state":""}`)
...
func TestHooks_ReviewerRoutesToReviewActivity(t *testing.T) {
	t.Setenv("OPERATOR_REVIEW_SESSION_ID", "review-7")
	t.Setenv("OPERATOR_REVIEW_WORKER_SESSION_ID", "worker-7")
	t.Setenv("OPERATOR_REVIEW_HARNESS", "codex")
	cfg := setConfigEnv(t)
...
func TestHooks_ReviewerRoutingTakesPrecedenceOverWorkerSession(t *testing.T) {
	t.Setenv("OPERATOR_REVIEW_SESSION_ID", "review-7")
	t.Setenv("OPERATOR_REVIEW_WORKER_SESSION_ID", "worker-context-only")
	t.Setenv("OPERATOR_SESSION_ID", "worker-7")
	cfg := setConfigEnv(t)
...
func TestHooks_NotificationReportsBlocked(t *testing.T) {
	t.Setenv("OPERATOR_SESSION_ID", "opr-7")
	cfg := setConfigEnv(t)
	srv, capture := activityServer(t, http.StatusOK, `{"ok":true,"sessionId":"opr-7","state":"blocked"}`)
	writeRunFileFor(t, cfg, srv)
	...
	if capture.path != "/api/v1/sessions/opr-7/activity" {
		t.Errorf("path = %q, want /api/v1/sessions/opr-7/activity", capture.path)
	}
...
func TestHooks_IdlePromptReportsIdle(t *testing.T) {
	t.Setenv("OPERATOR_SESSION_ID", "opr-7")
	cfg := setConfigEnv(t)
	srv, capture := activityServer(t, http.StatusOK, `{"ok":true,"sessionId":"opr-7","state":"idle"}`)
	writeRunFileFor(t, cfg, srv)
...
func TestHooks_ThreadsRuntimeLaunchID(t *testing.T) {
	t.Setenv("OPERATOR_SESSION_ID", "opr-7")
	t.Setenv("OPERATOR_RUNTIME_LAUNCH_ID", "launch-3")
	cfg := setConfigEnv(t)
	srv, capture := activityServer(t, http.StatusOK, `{"ok":true}`)
	writeRunFileFor(t, cfg, srv)
...
func TestHookConversationFactsExcludesOperatorCoordinationUserTurns(t *testing.T) {
	for _, prompt := range []string{
		...
		wantAssistant := "ok"
		if strings.HasPrefix(prompt, "<opr-handoff-request") {
			wantAssistant = ""
		}
...
func TestHooks_AgySessionStartReportsConversationID(t *testing.T) {
	t.Setenv("OPERATOR_SESSION_ID", "opr-7")
	cfg := setConfigEnv(t)
	promptDir := filepath.Join(cfg.dataDir, "prompts", "opr-7")
	if err := os.MkdirAll(promptDir, 0o750); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(promptDir, "system.md"), []byte("follow Operator standing instructions\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	srv, capture := activityServer(t, http.StatusOK, `{"ok":true}`)
	writeRunFileFor(t, cfg, srv)
...
func TestHooks_DevinSessionStartInjectsSystemPromptContext(t *testing.T) {
	...
	if got.HookSpecificOutput.AdditionalContext != "follow Operator standing instructions" {
...
func TestHooks_RejectsMalformedSessionID(t *testing.T) {
	t.Setenv("OPERATOR_SESSION_ID", "../etc/passwd")
	cfg := setConfigEnv(t)
...
func TestHooks_NoSessionIDIsNoOp(t *testing.T) {
	...
	if capture.hits != 0 {
		t.Errorf("expected no daemon call for a non-Operator session, got %d", capture.hits)
	}
}
...
// $OPERATOR_DATA_DIR/hooks.log — and a delivered hook must not write the file at all.
func TestHooks_DeliveryFailureGoesToHooksLog(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			wantLog: true,
			wantIn:  []string{"opr hooks claude-code session-end", "session=opr-7"},
		},
		{
...
func TestHooks_HooksLogTruncatesPastCap(t *testing.T) {
	...
	if !strings.Contains(string(data), "opr hooks claude-code session-end") {
