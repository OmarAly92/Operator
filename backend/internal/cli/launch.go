...
import (
	...
	"github.com/OmarAly92/operator/backend/internal/agentlaunch"
)

func newLaunchCommand(ctx *commandContext) *cobra.Command {
	return &cobra.Command{
		Use:    "launch",
		Short:  "Launch an Operator-managed agent process (internal)",
		Hidden: true,
		Args:   noArgs,
...
func (c *commandContext) launchAgent(ctx context.Context) error {
	...
	if specPath == "" {
		return errors.New("launch: OPERATOR_LAUNCH_SPEC is required")
	}
	...
	if launchErr != nil {
		_, _ = fmt.Fprintf(c.deps.Err, "\r\n[opr launch] agent process exited: %v\r\n", launchErr)
	}
