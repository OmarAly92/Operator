...
func newProjectAddCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Register a local git repo as a project so sessions can be spawned in it.\n\n" +
			...
			"Operator initializes/adopts the parent as the root repo and gitignores children.",
		Args: noArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if opts.path == "" {
...
func newProjectSetConfigCommand(ctx *commandContext) *cobra.Command {
	...
	f.StringVar(&opts.permission, "permission", "", "Permission mode: default, accept-edits, plan, auto, bypass-permissions")
...
func writeProjectList(cmd *cobra.Command, projects []projectSummary) error {
	...
	if len(projects) == 0 {
		...
		_, err := fmt.Fprintln(out, "Run `opr project add --path <path>` to register one.")
		return err
	}
