...
func newSpawnCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Spawn an agent session (worker or orchestrator) in a registered project.\n\n" +
			...
			"fresh isolated workspace. Git projects use worktrees; Scratch uses an Operator-managed directory.",
		Args: noArgs,
	...
	f := cmd.Flags()
	// --agent is an alias for --harness so the more intuitive `opr spawn --agent
	...
	f.StringVar(&opts.project, "project", "", "Project id to spawn the session in (default: OPERATOR_PROJECT_ID, current registered repo, or Scratch when it is the only project)")
	...
	f.StringVar(&opts.branch, "branch", "", "Branch for git project sessions (default: opr/<session-id>/root; unsupported for Scratch)")
...
func (c *commandContext) resolveSpawnProject(ctx context.Context, explicit string) (projectDetails, error) {
	...
	if id := strings.TrimSpace(os.Getenv("OPERATOR_PROJECT_ID")); id != "" {
		return c.fetchProjectDetails(ctx, id)
	}
	if sessionID := strings.TrimSpace(os.Getenv("OPERATOR_SESSION_ID")); sessionID != "" {
...
func (c *commandContext) resolveProjectFromSession(ctx context.Context, sessionID string) (projectDetails, error) {
	...
	if err != nil {
		return projectDetails{}, usageError{fmt.Errorf("project could not be resolved from OPERATOR_SESSION_ID %q; pass --project", sessionID)}
	}
...
func (c *commandContext) preflightSpawnAgentAuth(ctx context.Context, cmd *cobra.Command, agentID string) error {
	...
	if !state.supported {
		return fmt.Errorf("agent %q is not supported by this daemon; pass a supported --agent or run `opr agent ls`", agentID)
	}
