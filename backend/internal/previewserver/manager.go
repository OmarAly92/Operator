...
const (
	...
	ConfigPath              = ".operator/launch.json"
...
const (
	...
	// StateStopping indicates that Operator is terminating the preview process.
...
// Configuration is one named server entry in .operator/launch.json. ${PORT} is
// expanded in runtimeArgs, url, and env values after Operator chooses the port.
...
type serverRun struct {
	status Status
	cmd    *exec.Cmd
	// startTime pins cmd's PID to the process Operator launched (kernel start time,
...
// Manager supervises at most one managed preview server per Operator session.
type Manager struct {
	log    *slog.Logger
	client *http.Client
...
// Stop terminates the exact process tree Operator launched for the session.
...
func (m *Manager) stop(ctx context.Context, sessionID domain.SessionID) (Status, error) {
	...
	case <-done:
		...
		// Operator's preview, so nothing is killed: a leaked preview process is
...
func (m *Manager) waitForExit(sessionID domain.SessionID, run *serverRun) {
	...
	// provably Operator's. No escalation happens here: descendants the dead root
...
func (m *Manager) failAndStop(
	...
	if cmd != nil {
		...
		select {
		case <-run.done:
			// Reaped: the PID is no longer provably Operator's, nothing to escalate.
...
func previewEnvironment(
	...
	env = append(env,
		...
		"OPERATOR_PREVIEW_PORT="+strconv.Itoa(port),
		"OPERATOR_SESSION_ID="+string(sessionID),
	)
	return env
}
