...
// ("opr.cli.usage_errors") both gets folded into one rollup AND that the
...
func TestTelemetryWiringAggregatesUsageErrorsAndPreservesCountOnTheWire(t *testing.T) {
	...
	select {
	case req := <-requests:
		if got := req["event"]; got != "opr.cli.usage_errors" {
			t.Fatalf("event = %#v, want opr.cli.usage_errors (aggregatedEventNames entry must match the real emit site)", got)
		}
