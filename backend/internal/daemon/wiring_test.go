...
func TestWiring_StartSessionBuildsSessionService(t *testing.T) {
	...
	svc, reviewSvc, lc, err := startSession(context.Background(), cfg, rt, store, lcm, messenger, telemetryadapter.NoopSink{}, agents, nil, nil, nil, log)
	if err != nil {
...
func TestWiring_StartSessionSpawnsScratchWithoutGitRepo(t *testing.T) {
	...
	svc, _, _, err := startSession(context.Background(), cfg, runtime, store, lcm, messenger, telemetryadapter.NoopSink{}, agents, nil, nil, nil, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
...
func TestStartSession_SpawnDoesNotPanicWhenNoTrackerToken(t *testing.T) {
	...
	svc, _, _, err := startSession(context.Background(), cfg, rt, store, lcm, messenger, telemetryadapter.NoopSink{}, agents, nil, nil, nil, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
