...
type observedTmuxRuntime struct {
	*tmux.Runtime
	beforeDestroy func(string)
}

func (r *observedTmuxRuntime) Destroy(ctx context.Context, handle ports.RuntimeHandle) error {
...
func newShellBlocksHarness(t *testing.T, appRunID string) *shellBlocksHarness {
	...
	tmuxTmpDir, err := os.MkdirTemp("/tmp", "opr-shell-blocks-tmux-")
	if err != nil {
		t.Fatal(err)
	}
	...
	runtime := &observedTmuxRuntime{Runtime: tmux.New(tmux.Options{
		Shell:      "/bin/sh",
...
func tmuxClients(t *testing.T, handleID string) int {
	t.Helper()
	out, err := exec.Command("tmux", "list-clients", "-F", "#{client_session}").CombinedOutput()
	if err != nil {
		if strings.Contains(string(out), "no server running") || strings.Contains(string(out), "no current client") {
			return 0
		}
		t.Fatalf("list tmux clients: %v: %s", err, out)
	}
	count := 0
...
func TestShellBlocksZeroClientHistoryUsesBootstrapRecipe(t *testing.T) {
	...
	if got := tmuxClients(t, h.terminal.HandleID); got != 0 {
		t.Fatalf("tmux clients before commands = %d, want 0", got)
	}
	...
	defer cancel()
	defer stream.Close()
	waitForShellBlocks(t, "one attached tmux client", func() bool {
		return tmuxClients(t, h.terminal.HandleID) == 1
	})
...
func TestShellBlocksTwoClientsProduceOneRowAndEvent(t *testing.T) {
	...
	defer second.Close()
	waitForShellBlocks(t, "two attached tmux clients", func() bool {
		return tmuxClients(t, h.terminal.HandleID) == 2
	})
