...
var (
	...
	// version Operator does not support.
	...
	// ErrChatNoActiveTurn means an interrupt found nothing to cancel — either Operator
	...
	// Operator invented is not consent, and consuming the request on a bad decision
...
// ChatCapability names something a driver may or may not be able to do. Operator gates
// features on these rather than on the harness name.
type ChatCapability string

// The capabilities Operator currently checks.
const (
	...
	// ChatCapabilityRename means the thread carries a title Operator can set.
	...
	// prompt. Operator may still stage a worktree copy for durable/local context, but it
	...
	// permission for the provider to execute through Operator's terminal runtime.
...
// chatProductionFloor is the minimum a driver must support before Operator will let a
...
type ChatStartConfig struct {
	...
	// DataDir is Operator's state root. Provider bindings may write process-scoped
	// configuration beneath it, but must never use the worktree or an OS-default
	// application-data directory for Operator-owned state.
	DataDir string
	...
	// shell commands the agent runs. Operator passes a HookPATH-augmented copy so the
	// agent can invoke `opr` — that is how an orchestrator delegates.
	Env map[string]string
	// Model is optional; empty defers to the provider's configured default.
	Model string
	// Permissions is Operator's existing per-session approval policy. Drivers map it
	// onto their provider's native approval and sandbox settings.
	Permissions PermissionMode
	// SystemPrompt carries Operator's standing instructions for the session.
	SystemPrompt string
	...
	// worktrees; it is not a replacement for Operator's worktree ownership.
...
// server supplied by Operator. Exactly one transport is meaningful according to Type.
...
type ChatTurnSettings struct {
	...
	Effort string
	// Approval is Operator's permission mode for this turn. The driver maps it onto
...
//
// The list comes from the provider, never from a table in Operator. A hardcoded catalog
// is wrong within a week: models are added, renamed, hidden per account, and
// gated by entitlement the provider knows about and Operator does not.
...
// Operator's vocabulary prevents protocol DTOs from leaking out of the adapter.
...
// render: that is how a newly released agent feature reaches Operator without an Operator
...
type ChatAccount struct {
	...
	// expected to supply. Operator does not hold provider credentials, so this is
...
type ChatThreadState struct {
	// Status is Operator's neutral spelling of the provider's state.
	Status domain.ThreadStatus
	// WaitingOn are the provider's active flags in Operator's spelling.
	WaitingOn []string
...
type ChatSkill struct {
	...
	// can tell an Operator-provided skill from one the provider ships.
	Source string
}
...
// one of these simply does not offer it and Operator hides the affordance rather than
...
// only after that id is durably bound to Operator's turn row. This prevents a fast
...
type ChatDeferredTurnStarter interface {
	...
	// DiscardDeferredTurn releases a prepared turn when Operator could not persist its
...
// ChatInputMode distinguishes the two ACP elicitation shapes Operator supports.
...
// structured user input. Operator gates the route/UI on the negotiated capability.
...
const (
	...
	// Operator did not originally render (for example, work completed in the TUI).
	...
	// Kinds below carry provider signal Operator previously discarded. Each is modelled
	...
	// need for credentials Operator does not hold.
...
// is gone, matching how Operator already treats runtime probes.
const (
...
type ChatDriver interface {
	...
	// can be rejected before Operator commits resources.
...
type ChatConversation interface {
	// ProviderConversationID is the opaque identifier Operator persists so a later
