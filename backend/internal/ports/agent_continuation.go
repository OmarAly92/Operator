...
const (
	...
	// Operator learns it from the provider's lifecycle hooks.
...
// ContinuationCapabilities is the static fresh-conversation behavior Operator needs
...
// switch orchestration. Operator persists the returned id and passes it back through
...
// conflating it with the stable Operator session id. ConfigDir is the exact native
...
// a launch. Operator stores the returned path with the native-session binding so a
...
// even when Operator cannot find authoritative local state.
