...
// AdoptProviderTurn records a turn the provider started that Operator never dispatched.
//
...
//
// Idempotency is by provider turn rather than only by clientMessageID. A prompt Operator
...
func (s *Store) SettleTurn(
	...
	if errors.Is(err, sql.ErrNoRows) {
		// A turn Operator never recorded, e.g. one started by a previous controller
...
// NULL, because "Operator asked and there are none" is a different answer from "Operator never
// asked".
...
//
// A plan for a turn Operator never recorded happens after a restart, when a controller
...
// activity.
//
// Reports whether a row was found. A delta for an activity Operator has not recorded is
...
// SettleTurnByID records a terminal state for a turn Operator can name directly.
//
...
// message on first sight. The provider item id is the correlation key because Operator
...
func (s *Store) SettleAssistantMessage(
	...
	if errors.Is(err, sql.ErrNoRows) {
		// A message whose deltas Operator never saw — possible if it completed inside a
...
//
// Reports whether the turn was found. A diff for a turn Operator never recorded happens
...
//
// This is the Operator half of an operation the provider has already performed: the agent
	...
	err = s.inTx(ctx, "apply provider title", func(q *gen.Queries) error {
		rows, updateErr := q.ApplyConversationTitleToSession(ctx,
			gen.ApplyConversationTitleToSessionParams{
				...
				UpdatedAt:   now,
				ID:          session,
				// The witness: only a label Operator itself last wrote may be replaced.
...
// turnIDFor resolves a provider turn id to Operator's turn id, or nil when the turn is
...
// happen, and showing an empty meter for the first would be a claim Operator has not
// earned.
