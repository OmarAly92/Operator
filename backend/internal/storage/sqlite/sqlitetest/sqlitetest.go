...
// already contain opr.db. Tests that exercise migration behavior should continue
...
func buildTemplateDatabase() ([]byte, error) {
	dataDir, err := os.MkdirTemp("", "opr-sqlite-test-template-")
	if err != nil {
	...
	data, err := os.ReadFile(filepath.Join(dataDir, "opr.db"))
	if err != nil {
