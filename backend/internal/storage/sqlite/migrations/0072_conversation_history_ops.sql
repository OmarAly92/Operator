...
-- provider genuinely drops those turns from the history it reasons over, so Operator's
...
-- DELETE because Operator does not destroy durable facts. "This exchange happened and was
...
--   * rows with turn_id IS NULL stay visible. Those are Operator's own bookkeeping (usage,
--     controller errors), never agent memory, and hiding what Operator cannot prove
...
-- +goose StatementBegin
-- The thread title the provider reports, and the last one Operator pushed into
...
-- overridden the Operator label, since it is the name the conversation has in the
-- provider's own history. applied_title is the compare-and-set witness: Operator may
...
--
-- Empty applied_title means Operator has never named this session. Deliberately not a
