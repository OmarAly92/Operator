...
func reviewQueueText(spec LaunchSpec) string {
	...
	fmt.Fprintf(&b, "\nOperator created %d review tasks for this worker session. Review every queued PR, then submit all results together.\n\nReview task queue:\n", len(spec.ReviewQueue))
