...
func TestFileURLPreservesSpecialCharactersInEntryPath(t *testing.T) {
	raw := mustFileURL(t, "http://127.0.0.1:3001", "opr-1", "dist/my report #1.html")
	parsed, err := url.Parse(raw)
	if err != nil {
		t.Fatalf("parse FileURL: %v", err)
	}
...
func TestFileURLPreservesLeadingAndTrailingSpacesInEntryPath(t *testing.T) {
	raw := mustFileURL(t, "http://127.0.0.1:3001", "opr-1", "dist/ report.html ")
	parsed, err := url.Parse(raw)
	if err != nil {
		t.Fatalf("parse FileURL: %v", err)
	}
...
func TestFileURLRejectsHostnameOverDNSLimit(t *testing.T) {
	accepted := domain.SessionID(strings.Repeat("x", 141))
	...
	if len(parsed.Hostname()) != 252 {
		t.Fatalf("boundary hostname length = %d, want 252 (largest that fits under the 253 DNS limit)", len(parsed.Hostname()))
	}
	...
	_, err = FileURL("http://127.0.0.1:3001", domain.SessionID(strings.Repeat("x", 142)), "index.html")
...
func TestSessionIDFromHostRejectsHostnameOverDNSLimit(t *testing.T) {
	host := "opr-preview." + strings.Repeat("a.", 120) + "localhost:3001"
...
func TestStoredWorkspaceEntryPreservesLegacyAndRelativeTargets(t *testing.T) {
	for _, tc := range []struct {
		...
		{raw: "http://127.0.0.1:3001/api/v1/sessions/opr-1/preview/files/docs/report.html", want: "docs/report.html"},
		{raw: "docs/report.html", want: "docs/report.html"},
		{raw: " docs/report.html ", want: " docs/report.html "},
	} {
		got, ok := StoredWorkspaceEntry(tc.raw, "opr-1")
		if !ok || got != tc.want {
			t.Errorf("StoredWorkspaceEntry(%q) = %q, %v; want %q, true", tc.raw, got, ok, tc.want)
		}
	}
	if got, ok := StoredWorkspaceEntry("https://example.com/api/v1/sessions/opr-1/preview/files/docs/report.html", "opr-1"); ok {
...
func TestSessionIDFromHostRejectsOrdinaryHosts(t *testing.T) {
	for _, host := range []string{"127.0.0.1:3001", "localhost:3001", "opr-preview.invalid.localhost:3001", "example.com"} {
		if id, ok := SessionIDFromHost(host); ok {
