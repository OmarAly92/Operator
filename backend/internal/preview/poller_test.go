...
func TestPollerDoesNotOpenUntargetedWorkerEntry(t *testing.T) {
	...
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, "")}}
...
func TestPollerNormalizesExplicitWorkspaceEntrypoint(t *testing.T) {
	...
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, "dist/index.html")}}
	poller := NewPoller(svc, svc, "http://127.0.0.1:3001", PollerConfig{Logger: discardLogger()})
	...
	assertSets(t, svc.sets, previewSet{
		id:  "opr-1",
		url: mustFileURL(t, "http://127.0.0.1:3001", "opr-1", "dist/index.html"),
	})
}
...
func TestPollerRefreshesOnlyWhenEntrypointChanges(t *testing.T) {
	...
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, "index.html")}}
	poller := NewPoller(svc, svc, "http://127.0.0.1:3001", PollerConfig{Logger: discardLogger()})
...
func TestPollerRediscoverEntryAfterDeleteAndRecreate(t *testing.T) {
	...
	wantURL := mustFileURL(t, "http://127.0.0.1:3001", "opr-1", "index.html")
	assertSets(t, svc.sets, previewSet{id: "opr-1", url: wantURL})
...
func TestPollerDoesNotRestoreClearedPreviewAfterRestart(t *testing.T) {
	workspace := t.TempDir()
	writeFile(t, filepath.Join(workspace, "index.html"), "<main>hello</main>")
	sess := workerSession("opr-1", workspace, "")
...
func TestPollerDoesNotOverrideExplicitPreviewTarget(t *testing.T) {
	workspace := t.TempDir()
	writeFile(t, filepath.Join(workspace, "index.html"), "<main>hello</main>")
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, "file:///C:/tmp/other.html")}}
	poller := NewPoller(svc, svc, "http://127.0.0.1:3001", PollerConfig{Logger: discardLogger()})
...
func TestPollerMigratesLegacyWorkspacePreviewURL(t *testing.T) {
	...
	legacy := "http://127.0.0.1:3001/api/v1/sessions/opr-1/preview/files/docs/report.html"
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, legacy)}}
	poller := NewPoller(svc, svc, "http://127.0.0.1:3001", PollerConfig{Logger: discardLogger()})
	...
	assertSets(t, svc.sets, previewSet{
		id:  "opr-1",
		url: mustFileURL(t, "http://127.0.0.1:3001", "opr-1", "docs/report.html"),
	})
}
...
func TestPollerPreservesStoredRelativeEntry(t *testing.T) {
	...
	writeFile(t, filepath.Join(workspace, "docs", "report.html"), "<main>chosen report</main>")
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, "docs/report.html")}}
	poller := NewPoller(svc, svc, "http://127.0.0.1:3001", PollerConfig{Logger: discardLogger()})
...
func TestPollerRewritesStoredOriginToActualDaemonPortWithoutChangingEntry(t *testing.T) {
	...
	writeFile(t, filepath.Join(workspace, "index.html"), "<main>default</main>")
	writeFile(t, filepath.Join(workspace, "docs", "report.html"), "<main>chosen report</main>")
	old := mustFileURL(t, "http://127.0.0.1:3001", "opr-1", "docs/report.html")
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, old)}}
	...
	assertSets(t, svc.sets, previewSet{
		id:  "opr-1",
		url: mustFileURL(t, "http://127.0.0.1:49152", "opr-1", "docs/report.html"),
	})
}
...
func TestPollerKeepsMarkdownPreviewFreshOnceExplicitlySet(t *testing.T) {
	...
	svc := &fakePreviewSessions{sessions: []domain.SessionRecord{workerSession("opr-1", workspace, relative)}}
	poller := NewPoller(svc, svc, "http://127.0.0.1:3001", PollerConfig{Logger: discardLogger()})
	...
	assertSets(t, svc.sets, previewSet{
		id:  "opr-1",
		url: mustFileURL(t, "http://127.0.0.1:3001", "opr-1", relative),
	})
}
