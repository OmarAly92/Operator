...
// Scenarios for "what Operator shows is what actually happened".
//
// The provider is the authority on model context; these rows are the authority on
// what Operator renders and on delivery state. So the failure this file is looking for
...
// `opr send` and orchestrator-to-worker relay both go through this endpoint. A
// chat session has no pane to type into, so without a mode branch the send is
// refused as "missing runtime handles" and chat workers are unreachable by Operator's
// own automation.
func TestChatRelayThroughTheSendEndpointIsAttributedToAutomation(t *testing.T) {
	...
	// The endpoint answers 200 with an echo, the same shape `opr send` already
	...
	// Operator carried this on someone else's behalf; the timeline must say so rather
...
// Approvals are the one place the provider blocks on Operator: it holds its turn until
...
func TestChatApprovalRoundTrip(t *testing.T) {
	...
	send(t, d, session,
		...
		`Run this exact shell command and tell me its exit code: touch "$HOME/.operator-e2e-approval-probe"`,
		"approval-1")
	...
	// Operator must render that list rather than a fixed set, or it invents consent the
	...
	if status < 400 {
		t.Errorf("Operator accepted a decision the provider never offered (status %d)", status)
	} else if body.Code == "" {
		t.Errorf("refusal carried no error code: %+v", body)
	}
...
// The agent must be able to reach Operator's own CLI from inside a chat session. This is
// what makes chat orchestration possible at all: an orchestrator that cannot run
// `opr` can only talk, not delegate.
func TestChatAgentCanRunTheOperatorCLI(t *testing.T) {
	requireE2E(t)
	d := startDaemon(t, t.TempDir())
	...
	send(t, d, session,
		"Run the shell command `opr --version` and reply with its exact output, or reply NOT-FOUND if the command does not exist.",
		"opr-path")

	snap := d.awaitConversation(session, 3*time.Minute, "the opr probe to finish", func(s snapshot) bool {
		return terminal(s.Turns[len(s.Turns)-1].State)
	})
	answer := snap.assistantText()
	if contains(answer, "NOT-FOUND") || contains(answer, "command not found") {
		t.Fatalf("the agent could not run `opr` inside a chat session:\n%s", describe(snap))
	}
}
...
func TestChatProviderEventsAreArchived(t *testing.T) {
	...
	// detail payload that will not decode means Operator stored something it cannot read
...
// Model selection has to reach the provider, not just be accepted by Operator. The
...
func TestChatModelSelectionComesFromTheProviderAndAppliesToTheNextTurn(t *testing.T) {
	...
	// An approval mode outside Operator's vocabulary is refused rather than normalized:
