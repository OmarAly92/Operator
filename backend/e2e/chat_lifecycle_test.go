...
// and Operator reattaches to it, rather than replaying a transcript at a fresh agent and
...
func TestChatSurvivesADaemonRestartWithNativeContext(t *testing.T) {
	...
	// answer from the earlier turn without Operator replaying anything. If Operator were
...
// A message the user typed and Operator accepted must not vanish because the daemon
