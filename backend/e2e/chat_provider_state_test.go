...
// End-to-end coverage for the provider signal Operator used to drop.
//
...
func TestChatE2EProviderStateFromOneTurn(t *testing.T) {
	...
	if plan == nil {
		...
		for i, step := range plan.Steps {
			...
			default:
				t.Errorf("plan step %d status = %q, want Operator's own spelling", i, step.Status)
			}
		}
	...
	for _, row := range changes {
		...
		for _, file := range files {
			...
			// The provider spells the change kind as an object. Operator's own status is what
			// a client can actually read.
			switch file.Status {
			case "added", "modified", "deleted", "renamed":
			default:
				t.Errorf("changed file status = %q, want Operator's neutral spelling", file.Status)
			}
			if file.Patch != "" {
				sawPatch = true
			}
...
// detailFile mirrors what a client reads out of a file_change activity: Operator's own
