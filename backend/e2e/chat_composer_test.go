...
// user's own config and the repo's own files — neither of which Operator is told about.
...
func TestChatAttachmentIsStagedIntoTheWorktreeAndReadableByTheAgent(t *testing.T) {
	...
	path := staged.Paths[0]
	if !strings.HasPrefix(path, ".operator/attachments/") || !strings.HasSuffix(path, ".png") {
		t.Errorf("staged path = %q, want a .png under .operator/attachments/", path)
	}
	...
	// The images must not dirty the worktree: Operator derives session state from durable
	...
	for _, file := range files.Files {
		if strings.HasPrefix(file.Path, ".operator/attachments/") {
