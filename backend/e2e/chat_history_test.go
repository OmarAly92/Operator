...
//
// Rollback is the one worth running for real. Every unit test can prove Operator sends
...
// Operator's timeline does not show it either.
//
...
// history" from "the provider still has it and Operator is merely hiding rows".
func TestChatRollbackMakesTheAgentForget(t *testing.T) {
	...
	// Operator's side: the turn rows survive, marked; their prose is gone.
	marked := 0
	...
	if strings.Contains(answer, "ZEPHYRINE") {
		t.Errorf("the agent still remembers a turn Operator says was rolled back:\n%s", describe(asked))
	}
...
// still writing into would leave Operator's rows and the agent's memory describing
...
// The title round trip. Operator asks the provider to name the thread; the provider
