...
- Use **isolated mode** by default for implementation and destructive testing. Electron uses port `3002`, `~/.operator/dev/running.json`, `~/.operator/dev/data`, and `~/.operator/dev/electron`.
- Use **real-data mode** only when the user explicitly asks to see this machine's actual Operator projects or sessions. Start the checkout's dev daemon on the isolated dev port/run file while pointing `OPERATOR_DATA_DIR` at the real Operator data directory. This is a separate daemon process using real data; do not describe it as the installed app's daemon.
...
Use the full command paths and start times to distinguish this checkout from other Operator worktrees. On Windows, use `Get-CimInstance Win32_Process` and `Get-NetTCPConnection`; preserve the same exact-target rule.
...
```bash
cd frontend
env -u OPERATOR_DATA_DIR -u OPERATOR_RUN_FILE -u OPERATOR_PORT npm run dev
```
...
Remove-Item Env:OPERATOR_DATA_DIR, Env:OPERATOR_RUN_FILE, Env:OPERATOR_PORT -ErrorAction SilentlyContinue
npm run dev
```

### Real-data mode

First resolve and report the actual data directory. In an Operator worker session, `OPERATOR_DATA_DIR` normally already identifies it. Otherwise the repository default is the absolute path corresponding to `~/.operator/data`.
...
```bash
cd frontend
env -u OPERATOR_RUN_FILE -u OPERATOR_PORT OPERATOR_DATA_DIR="${OPERATOR_DATA_DIR:-$HOME/.operator/data}" npm run dev
```
...
```powershell
Set-Location frontend
$realAoData = if ($env:OPERATOR_DATA_DIR) { $env:OPERATOR_DATA_DIR } else { Join-Path $HOME ".operator/data" }
Remove-Item Env:OPERATOR_RUN_FILE, Env:OPERATOR_PORT -ErrorAction SilentlyContinue
$env:OPERATOR_DATA_DIR = $realAoData
npm run dev
```

Do not print unrelated environment variables: Operator sessions may carry credentials. It is safe to report only `OPERATOR_DATA_DIR`, `OPERATOR_RUN_FILE`, and `OPERATOR_PORT`.
...
If several Operator windows exist, identify the newest Electron main process belonging to this checkout. On macOS, foreground that exact PID only when needed:
...
- A renderer error such as `operatorBridge.<method> is not a function` almost always means the renderer hot-reloaded against a stale preload. Restart Electron before changing code to add guards around a bridge method that should exist.
...
4. Confirm the captured Electron PID and dev daemon listener are gone. Leave installed Operator and other worktrees untouched.

## Common mistakes

- `opr preview` controls the Operator Browser panel; it does not launch the desktop shell.
...
- Multiple dev instances share `~/.operator/dev/electron`; avoid running them concurrently because Chromium profile state can collide.
- An inherited `OPERATOR_DATA_DIR` changes dev mode from isolated data to real data. Always choose and report the mode instead of inheriting it accidentally.
