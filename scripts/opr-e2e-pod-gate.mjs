...
//
//   node scripts/opr-e2e-pod-gate.mjs --repo <owner/repo> --sha <sha> --tag <release-tag> --suite T0
...
//     needs no egress.
//   - Prints a final line: OPERATOR_VERDICT {"passed":true|false,...}
...
// outcome into an `opr-stable-gate` commit status. The verdict->exit-code->status
...
/**
 * Parse the pod's final OPERATOR_VERDICT line into { passed, infra }.
 *
 ...
 *
 * The pod may emit more than one OPERATOR_VERDICT line (e.g. a retry, or a later
...
export function parsePodVerdict(out) {
	const matches = [...(out ?? "").matchAll(/^.*OPERATOR_VERDICT (\{.*\})\s*$/gm)];
...
export function validateGateArgs({ apiKey, repo, tag } = {}) {
	...
	if (missing.length > 0) {
		throw new Error(`opr-e2e-pod-gate: missing required input(s): ${missing.join(", ")}`);
	}
}

// GitHub release Linux .deb URL for a tag. Forge publishes to v<version>; the
// asset is operator_<version>_amd64.deb (version = tag without "v").
export function releaseDebUrl(repo, tag) {
	const version = String(tag).replace(/^v/, "");
	return `https://github.com/${repo}/releases/download/${tag}/operator_${version}_amd64.deb`;
}
...
async function runPodSuite({ repo, tag, apiKey, suite, artifactsDir, timeoutMs = 20 * 60_000 }) {
	...
	try {
		...
		sandbox = await daytona.create({ snapshot: process.env.OPERATOR_DAYTONA_SNAPSHOT || "daytona-small" });
		...
		const suiteEnv = suite ? `OPERATOR_SUITE=${suite} ` : "";
		const r = await sandbox.process.executeCommand(
			`OPERATOR_DEB_PATH=/home/daytona/app.deb ${suiteEnv}bash /home/daytona/boot-real.sh`,
			"/home/daytona",
			undefined,
		...
		if (!podLog) podLog = `opr-e2e-pod-gate: run failed before the pod produced output: ${err.message}\n`;
		throw err;
	} finally {
...
async function main(argv) {
	...
	const artifactsDir =
		process.env.OPERATOR_ARTIFACTS_DIR || join(dirname(fileURLToPath(import.meta.url)), "..", "e2e-artifacts");
	const runUrl =
	...
	try {
		...
		console.error(`opr-e2e-pod-gate: run failed: ${err.message}`);
		...
		try {
			mkdirSync(artifactsDir, { recursive: true });
			writeFileSync(join(artifactsDir, "pod.log"), `opr-e2e-pod-gate: run failed (infra/setup): ${err.message}\n`, {
				flag: "wx",
			});
		} catch {
	...
	console.log(`OPERATOR_VERDICT ${JSON.stringify(verdict)}`);
