...
import { deriveGateOutcome, parseArgs, parsePodVerdict, validateGateArgs } from "./opr-e2e-pod-gate.mjs";
...
test("parsePodVerdict: app pass / app fail / infra / missing", () => {
	assert.deepEqual(parsePodVerdict('OPERATOR_VERDICT {"passed":true,"suite":"real-app-t0"}'), {
		passed: true,
		infra: false,
	});
	assert.deepEqual(parsePodVerdict('OPERATOR_VERDICT {"passed":false,"playwright_exit":1}'), {
		passed: false,
		infra: false,
	});
	// setup/toolchain failure must be flagged infra, not an app failure
	assert.deepEqual(parsePodVerdict('OPERATOR_VERDICT {"passed":false,"infra":true,"stage":"npm-playwright"}'), {
		passed: false,
		infra: true,
	});
	...
	assert.deepEqual(parsePodVerdict("OPERATOR_VERDICT {not json}"), { passed: false, infra: true });
});

test("parsePodVerdict: uses the LAST OPERATOR_VERDICT line, not the first", () => {
	...
	// later failure.
	const out = [
		'OPERATOR_VERDICT {"passed":true,"suite":"real-app-t0"}',
		"...some later retry / log output...",
		'OPERATOR_VERDICT {"passed":false,"suite":"real-app-t0","playwright_exit":1}',
	].join("\n");
...
test("parsePodVerdict: last verdict wins for infra and pass ordering too", () => {
	...
	assert.deepEqual(parsePodVerdict('OPERATOR_VERDICT {"passed":true}\nOPERATOR_VERDICT {"passed":false,"infra":true}'), {
		passed: false,
		infra: true,
	});
	// a genuine pass after an earlier infra line -> pass wins
	assert.deepEqual(parsePodVerdict('OPERATOR_VERDICT {"passed":false,"infra":true}\nOPERATOR_VERDICT {"passed":true}'), {
		passed: true,
		infra: false,
	});
	// trailing whitespace/newlines after the final line must not defeat selection
	assert.deepEqual(parsePodVerdict('OPERATOR_VERDICT {"passed":true}\nOPERATOR_VERDICT {"passed":false}\n\n'), {
		passed: false,
		infra: false,
	});
});
...
test("pod infra verdict maps to NEUTRAL, real app fail maps to RED", () => {
	...
	const setupFail = parsePodVerdict('OPERATOR_VERDICT {"passed":false,"infra":true}');
	...
	const appFail = parsePodVerdict('OPERATOR_VERDICT {"passed":false}');
