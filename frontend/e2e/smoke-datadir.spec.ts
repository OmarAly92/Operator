...
// ~/.operator layout or the real daemon/storage. The on-disk boundary is exercised only
// in the packaged-app pod gate (#2697), which today runs a boot-level smoke (app
// launches, daemon ready), NOT the full ~/.operator layout assertion — that stays in
// the pod data-dir script as future per-case work.
//
// The on-disk checks (Electron userData resolves under ~/.operator/electron, daemon
// data under ~/.operator, no OS-default app-data writes, OPERATOR_DATA_DIR override) run in
...
// the ~/.operator filesystem assertions stay in the pod data-dir script.
