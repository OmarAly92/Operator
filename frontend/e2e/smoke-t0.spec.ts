...
// (VITE_NO_ELECTRON=1) with an injected `window.operator` (installFakeBridge /
...
test("renderer: reflects a ready daemon (data dir + config initialized) @T0 @INS", async ({ page }) => {
	...
	// advertises a port). Asserting the on-disk ~/.operator layout itself belongs to
...
test("renderer: route nav home to board to session detail and back @T0 @BRD", async ({ page }) => {
	...
	// → project board
	await page.locator('[data-sidebar="menu-button"]').filter({ hasText: "opr-demo" }).first().click();
	await expect(page).toHaveURL(/projects\/opr-demo/);
	await expect(page.getByTestId("board")).toBeVisible();
