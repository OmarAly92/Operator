...
type RevealObserverWindow = Window & {
	__oprRevealFrame?: number;
	__oprRevealFrames?: string[];
	__oprRevealObserver?: MutationObserver;
	__oprRevealSamples?: RevealSample[];
};
...
function activeBufferAtBottom(page: Page): Promise<boolean> {
	return activeTerminal(page)
		...
		.evaluate((element) => {
			const terminal = (element as HTMLElement & { __oprXtermForTest?: TestXterm }).__oprXtermForTest;
...
async function muxStats(page: Page): Promise<FakeTerminalMuxStats> {
	return page.evaluate(() => window.__oprFakeTerminalMux!.stats());
}

async function observeNextReveal(container: Locator): Promise<void> {
	await container.evaluate((element) => {
		const state = window as RevealObserverWindow;
		state.__oprRevealObserver?.disconnect();
		if (state.__oprRevealFrame !== undefined) {
			cancelAnimationFrame(state.__oprRevealFrame);
		}
		const samples: RevealSample[] = [];
		const framePhases: string[] = [];
		state.__oprRevealSamples = samples;
		state.__oprRevealFrames = framePhases;
		const captureVisibleSample = () => {
			const host = element.querySelector<HTMLElement>("[aria-label='Session terminal']");
			const viewport = host?.querySelector<HTMLElement>(".xterm-viewport");
			const terminal = (host as (HTMLElement & { __oprXtermForTest?: TestXterm }) | null)
				?.__oprXtermForTest;
			if (
		...
		const sampleFrame = () => {
			...
			if (phase === "visible") {
				...
				state.__oprRevealFrame = undefined;
				return;
			}
			state.__oprRevealFrame = requestAnimationFrame(sampleFrame);
		};
		state.__oprRevealFrame = requestAnimationFrame(sampleFrame);
		...
		state.__oprRevealObserver = observer;
	});
}

async function revealSamples(page: Page): Promise<RevealSample[]> {
	return page.evaluate(() => (window as RevealObserverWindow).__oprRevealSamples ?? []);
}

async function revealFramePhases(page: Page): Promise<string[]> {
	return page.evaluate(() => (window as RevealObserverWindow).__oprRevealFrames ?? []);
}
...
test.describe("retained terminal viewport", () => {
	test("retains the first of six live sessions with zero reopen and reveals its latest output", async ({
		...
		await page.evaluate(
			(handleId) =>
				window.__oprFakeTerminalMux!.emit(
					handleId,
		...
		await openSession(page, sessionB.title);
		await page.evaluate(
			(handleId) => window.__oprFakeTerminalMux!.emit(handleId, "\r\nA frame-sensitive bottom output"),
			handleA,
		);
		await observeNextReveal(parkedA);
		await openSession(page, sessionA.title);
	...
	test("suppresses parked resize and scroll storms during rapid A-B-A switching", async ({
		...
		await viewport.evaluate((element) => {
			(element as HTMLElement & { __oprScrollEvents?: number }).__oprScrollEvents = 0;
			element.addEventListener("scroll", () => {
				const tracked = element as HTMLElement & { __oprScrollEvents?: number };
				tracked.__oprScrollEvents = (tracked.__oprScrollEvents ?? 0) + 1;
			});
		});
		...
		await page.evaluate((handleId) => {
			window.__oprFakeTerminalMux!.emit(handleId, "\r\nA output during return");
		}, handleA);
		await expect.poll(() => activeBufferAtBottom(page)).toBe(true);
		await expect
		...
		expect(
			await activeViewport(page).evaluate(
				(element) => (element as HTMLElement & { __oprScrollEvents?: number }).__oprScrollEvents ?? 0,
			),
	...
	test("handles empty and short replay, blocks hidden input, and retains xterm through reconnect", async ({ page }) => {
		...
		await page.evaluate((handleId) => window.__oprFakeTerminalMux!.disconnect(handleId), handleA);
		...
		await page.evaluate((ids) => {
			for (const id of ids) window.__oprFakeAgent!.removeWorker(id);
