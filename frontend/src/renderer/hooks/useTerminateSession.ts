...
export function useTerminateSession(options: TerminateSessionOptions = {}) {
	...
	return useMutation({
		...
		mutationFn: async (session: WorkspaceSession) => {
			void captureRendererEvent("opr.renderer.session_kill_requested", { project_id: session.workspaceId });
		...
		onSuccess: async (_data, session) => {
			void captureRendererEvent("opr.renderer.session_kill_succeeded", { project_id: session.workspaceId });
			await queryClient.invalidateQueries({ queryKey: workspaceQueryKey });
			options.onSuccess?.(session);
		},
		onError: (_error, session) => {
			void captureRendererEvent("opr.renderer.session_kill_failed", { project_id: session.workspaceId });
		},
	});
}
