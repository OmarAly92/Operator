...
describe("spawnOrchestrator", () => {
	...
	it("emits the requested + succeeded triad keyed by source", async () => {
		...
		expect(captureMock).toHaveBeenCalledWith("opr.renderer.orchestrator_spawn_requested", {
			project_id: "proj",
			source: "sidebar",
		});
		expect(captureMock).toHaveBeenCalledWith("opr.renderer.orchestrator_spawn_succeeded", {
			project_id: "proj",
			source: "sidebar",
		});
	});
	...
	it("emits the failed event and rethrows when the daemon rejects the spawn", async () => {
		...
		expect(captureMock).toHaveBeenCalledWith("opr.renderer.orchestrator_spawn_failed", {
			project_id: "proj",
			source: "topbar",
		});
		expect(captureMock).not.toHaveBeenCalledWith("opr.renderer.orchestrator_spawn_succeeded", expect.anything());
	});
