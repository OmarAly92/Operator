...
describe("telemetry sanitizers", () => {
	it("isolates anonymous Operator installation identity from persisted PostHog person state", () => {
	...
	it("honours the supervisor deny list, by either internal or exported name", () => {
		expect(isDeniedEvent("opr.app.active", ["opr.v2.app.active"])).toBe(true);
		expect(isDeniedEvent("opr.app.active", ["opr.app.active"])).toBe(true);
		expect(isDeniedEvent("opr.app.active", ["  OPR.V2.APP.ACTIVE  "])).toBe(true);
		expect(isDeniedEvent("opr.renderer.route_viewed", ["opr.renderer.*"])).toBe(true);
		expect(isDeniedEvent("$exception", ["$exception"])).toBe(true);

		expect(isDeniedEvent("opr.session.spawned", ["opr.renderer.*"])).toBe(false);
		expect(isDeniedEvent("opr.app.active", [])).toBe(false);
	...
	it("allowlists only counts and the fixed agent-id list on agent inventory", async () => {
		const safe = await sanitizeRendererProperties("opr.renderer.agents_available", {
			installed_count: 2,
			authorized_count: 1,
			supported_count: 23,
	...
	it("allowlists only enum-like fields on update events", async () => {
		const safe = await sanitizeRendererProperties("opr.renderer.update_failed", {
			to_version: "0.11.2",
			error_category: "network",
			phase: "download",
			trigger: "automatic",
			// Must be dropped: raw updater text can carry feed URLs and local paths.
			message: "EACCES /Users/someone/Library/Caches/opr-updater",
		...
		const bogus = await sanitizeRendererProperties("opr.renderer.update_failed", { phase: "sideload" });
		expect(bogus.phase).toBeUndefined();
	});

	it("reports a support submission with only the destination and outcome", async () => {
		const safe = await sanitizeRendererProperties("opr.renderer.support_submitted", {
			destination: "discord",
			outcome: "succeeded",
	...
	it("reports the support open with no properties at all", async () => {
		const safe = await sanitizeRendererProperties("opr.renderer.support_opened", {
	...
	it("reports the mobile connect open with only the bridge state", async () => {
		const safe = await sanitizeRendererProperties("opr.renderer.mobile_connect_opened", {
			bridge_enabled: true,
	...
	it("reports the bridge toggle without the pairing payload", async () => {
		const safe = await sanitizeRendererProperties("opr.renderer.mobile_bridge_toggled", {
			enabled: true,
			outcome: "succeeded",
			password: "hunter2secret",
			host: "192.168.1.20",
	...
	it("disables every billable PostHog product Operator does not consume", () => {
		const config = buildPostHogConfig("ins_stable-install-id");
		...
		expect(config.disable_session_recording).toBe(true);
		// Operator reads no flags and ships no surveys, and /flags requests are billed
	...
	it("emits the stable Operator installation id without creating a person profile", () => {
		...
		try {
			const captured = client.capture("opr.app.active", { channel: "renderer" });
	...
	it("uses v2 PostHog event names for streams that have noisy legacy producers", () => {
		expect(postHogEventName("opr.app.active")).toBe("opr.v2.app.active");
		expect(postHogEventName("opr.renderer.route_viewed")).toBe("opr.v2.renderer.route_viewed");
		expect(postHogEventName("opr.renderer.api_error")).toBe("opr.v2.renderer.api_error");
		expect(postHogEventName("opr.session.spawned")).toBe("opr.session.spawned");
	});
	...
	it("hashes renderer ids and drops raw route identifiers", async () => {
		const props = await sanitizeRendererProperties("opr.renderer.project_removed", { project_id: "demo-project" });
		expect(props).toHaveProperty("project_id_hash");
		expect(props).not.toHaveProperty("project_id");

		const routeProps = await sanitizeRendererProperties("opr.renderer.route_viewed", {
	...
	it("keeps only the renderer channel on app active events", async () => {
		const props = await sanitizeRendererProperties("opr.app.active", {
			channel: "renderer",
		...
		expect(await sanitizeRendererProperties("opr.app.active", { channel: "cli" })).toEqual({});
	});

	it("keeps only bounded session-state fallback diagnostics", async () => {
		expect(
			await sanitizeRendererProperties("opr.renderer.session_state_unknown", {
				field: "status",
				reason: "unrecognized",
	...
	it("hashes project ids and drops everything else on CTA triads", async () => {
		const triads = [
			"opr.renderer.task_create_requested",
			"opr.renderer.task_create_succeeded",
			"opr.renderer.task_create_failed",
			"opr.renderer.session_kill_requested",
			"opr.renderer.session_kill_succeeded",
			"opr.renderer.session_kill_failed",
			"opr.renderer.settings_save_requested",
			"opr.renderer.settings_save_succeeded",
			"opr.renderer.settings_save_failed",
		];
	...
	it("keeps only the source enum on orchestrator_spawn events", async () => {
		const props = await sanitizeRendererProperties("opr.renderer.orchestrator_spawn_requested", {
			project_id: "demo-project",
			source: "board",
		});
		...
		const badSource = await sanitizeRendererProperties("opr.renderer.orchestrator_spawn_failed", {
			project_id: "demo-project",
	...
	it("keeps every whitelisted spawn source (the shared ORCHESTRATOR_SPAWN_SOURCES list)", async () => {
		for (const source of ORCHESTRATOR_SPAWN_SOURCES) {
			const props = await sanitizeRendererProperties("opr.renderer.orchestrator_spawn_succeeded", {
				project_id: "demo-project",
				source,
			});
			expect(Object.keys(props).sort()).toEqual(["project_id_hash", "source"]);
	...
	it("keeps only enum values on notification events", async () => {
		expect(await sanitizeRendererProperties("opr.renderer.notification_opened", { target: "pr" })).toEqual({
			target: "pr",
		});
		expect(await sanitizeRendererProperties("opr.renderer.notification_opened", { target: "http://x" })).toEqual({});
		expect(await sanitizeRendererProperties("opr.renderer.notification_mark_read_requested", { scope: "all" })).toEqual({
			scope: "all",
		});
		expect(await sanitizeRendererProperties("opr.renderer.notification_mark_read_succeeded", { scope: "all" })).toEqual({
			scope: "all",
		});
		expect(await sanitizeRendererProperties("opr.renderer.notification_mark_read_failed", { scope: "all" })).toEqual({
			scope: "all",
		});
		expect(
			await sanitizeRendererProperties("opr.renderer.notification_mark_read_requested", { scope: "everything" }),
		).toEqual({});
	});

	it("whitelists coarse daemon failure fields and drops messages", async () => {
		const props = await sanitizeRendererProperties("opr.renderer.daemon_failure", {
			daemon_state: "error",
			code: "spawn_failed",
			exit_code: 1,
			signal: "SIGKILL",
			message: "spawn /Users/alice/opr failed",
		});
	...
	it("whitelists normalized api_error fields", async () => {
		const props = await sanitizeRendererProperties("opr.renderer.api_error", {
	...
	it("keeps only the reason enum on terminal_attach_failed", async () => {
		expect(await sanitizeRendererProperties("opr.renderer.terminal_attach_failed", { reason: "open_timeout" })).toEqual({
			reason: "open_timeout",
		});
		expect(
			await sanitizeRendererProperties("opr.renderer.terminal_attach_failed", { reason: "something else" }),
		).toEqual({});
	});
});
...
describe("reserveCapture", () => {
	it("allows up to 5 captures of a name within a minute", () => {
		...
		for (let i = 0; i < 5; i += 1) {
			expect(reserveCapture("opr.renderer.route_viewed", start + i)).toBe(true);
		}
		expect(reserveCapture("opr.renderer.route_viewed", start + 5)).toBe(false);
	});

	it("tracks each name in its own window", () => {
		const start = 2_000_000;
		for (let i = 0; i < 5; i += 1) {
			reserveCapture("opr.renderer.route_viewed", start + i);
		}
	...
	it("resets the burst window once a minute elapses", () => {
		...
		for (let i = 0; i < 5; i += 1) {
			reserveCapture("opr.renderer.loaded", start + i);
		}
		expect(reserveCapture("opr.renderer.loaded", start + 60_001)).toBe(true);
	});

	it("caps the daily total even when calls are paced under the burst limit", () => {
		const name = "opr.renderer.paced_loop";
	...
	it("resets the daily ceiling after 24 hours", () => {
		const name = "opr.renderer.daily_reset";
...
describe("daily active heartbeat", () => {
	...
	it("treats a slot record left by an older build as today already reported", () => {
		...
		const storage = memoryStorage();
		storage.setItem("opr.telemetry.activeSlotsByDate", JSON.stringify({ date: "2026-07-12", slots: [0] }));
