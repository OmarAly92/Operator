...
vi.mock("../lib/bridge", () => ({ operatorBridge: { appState: { getMigration, setMigration } } }));
...
beforeEach(() => {
	...
	getMock.mockResolvedValue({ data: { available: true, legacyRoot: "/home/u/.operator" }, error: undefined });
...
describe("MigrationPopup", () => {
	it("shows when a legacy install is available and the marker is pending", async () => {
		renderPopup();
		expect(await screen.findByText(/Import projects from your earlier Operator/i)).toBeInTheDocument();
		expect(screen.getByText("/home/u/.operator")).toBeInTheDocument();
	});
	...
	it("renders nothing when the marker is declined", async () => {
		...
		expect(screen.queryByText(/Import projects from your earlier Operator/i)).not.toBeInTheDocument();
		expect(getMock).not.toHaveBeenCalled();
	});

	it("Proceed imports, marks completed, and retires", async () => {
		renderPopup();
		await screen.findByText(/Import projects from your earlier Operator/i);
		...
		await waitFor(() => expect(screen.queryByText(/Import projects from your earlier Operator/i)).not.toBeInTheDocument());
	});
