...
describe("ShellTerminalTab rename", () => {
	it("commits a new title on Enter", () => {
		...
		fireEvent.doubleClick(screen.getByRole("tab", { name: "opr" }));
	...
	it("discards on Escape and leaves the title unchanged", () => {
		...
		expect(screen.getByRole("tab", { name: "opr" })).toBeInTheDocument();
	});
	...
	it("selects the tab on single click", () => {
		...
		fireEvent.click(screen.getByRole("tab", { name: "opr" }));
		expect(onSelect).toHaveBeenCalled();
	});

	it("selects from the full connected pill, not only its title", () => {
		const { onSelect } = renderTab({ appearance: "connected" });
		fireEvent.click(screen.getByRole("tab", { name: "opr" }).parentElement as HTMLElement);
		expect(onSelect).toHaveBeenCalledOnce();
	});

	it("keeps the close affordance visible on an active connected tab", () => {
		renderTab({ appearance: "connected", isActive: true });
		expect(screen.getByRole("button", { name: "Close terminal opr" })).toHaveClass("w-control-sm", "opacity-100");
		expect(screen.getByRole("button", { name: "Close terminal opr" })).not.toHaveClass("absolute");
		expect(screen.getByRole("tab", { name: "opr" })).toHaveAttribute("aria-selected", "true");
	});
	...
	it("uses a compact fixed width and shrinks its title around the sibling close affordance", () => {
		...
		expect(screen.getByRole("button", { name: "Close terminal opr" })).toHaveClass(
			"w-0",
			"opacity-0",
		...
		expect(screen.getByRole("button", { name: "Close terminal opr" })).not.toHaveClass("absolute");
		expect(screen.getByRole("tab", { name: "opr" })).toHaveClass("w-full", "min-w-0", "text-left");
		expect(screen.getByRole("tab", { name: "opr" }).parentElement).toHaveClass(
			"grid",
	...
	it("uses a neutral active surface with a strong foreground selection line", () => {
		...
		expect(screen.getByRole("tab", { name: "opr" }).parentElement).not.toHaveClass("rounded-md");
		expect(screen.getByRole("tab", { name: "opr" }).parentElement).not.toHaveClass("before:bg-accent");
		expect(screen.getByRole("tab", { name: "opr" }).parentElement).not.toHaveClass("after:h-px");
	});

	it("optically centers the auxiliary terminal glyph with its label", () => {
		renderTab({ appearance: "connected" });

		expect(screen.getByRole("tab", { name: "opr" }).parentElement?.querySelector("svg")).toHaveClass("translate-y-px");
	});
});
...
describe("ShellTerminalTab rename gesture per platform", () => {
	it("macOS/Linux: double-click enters edit, right-click does not", () => {
		...
		const tab = screen.getByRole("tab", { name: "opr" });
