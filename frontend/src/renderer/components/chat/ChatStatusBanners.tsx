 ...
 * into `system_error` looks, from Operator's side, like an agent that has gone quiet.
 *
 ...
 * with the agent's own CLI, not with Operator, which is exactly why the daemon could not
 * fix this itself.
 */
				...
				<p className="text-[11px] leading-relaxed text-muted-foreground">
					{command ? (
						<>
							...
							in a terminal, then send your message again. Operator holds no credentials of its own.
						</>
					) : (
						<>
							Sign in with the agent&rsquo;s own CLI, then send your message again. Operator holds no
 ...
 * confused: the controller is Operator's connection to the agent process, this is what the
	...
	const copy =
		status === "system_error"
			? {
					...
					body: "The provider reported a fault in this thread, not in Operator's connection to it. New turns will usually fail; the conversation and the worktree are kept.",
				}
			: {
					title: "The agent closed this thread",
					body: "The provider dropped the conversation on its side. Operator kept the history, but the agent no longer holds it.",
				};
