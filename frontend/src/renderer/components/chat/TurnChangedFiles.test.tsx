...
describe("TurnChangedFiles", () => {
	...
	it("says the list was cut rather than presenting it as the whole change", async () => {
		...
		await userEvent.click(screen.getByRole("button"));
		expect(screen.getByText(/changed more files than Operator lists/i)).toBeInTheDocument();
	});
...
describe("ActivityRow command output", () => {
	...
	it("warns when output hit the storage cap", async () => {
		...
		await userEvent.click(screen.getByRole("button"));
		expect(screen.getByText(/printed more than Operator stores/i)).toBeInTheDocument();
	});
		...
		render(
			<ActivityRow
				activity={commandActivity(
					{
						command:
							"sed -n '1,240p' ~/.operator/dev/data/skills/using-opr/SKILL.md && sed -n '1,240p' ~/.operator/dev/data/skills/other/SKILL.md",
