...
export interface ChatWorkspaceProps {
	...
	sessionTitle?: string;
	/** The Operator role using this shared conversation surface. */
	...
	shellError?: string;
	/** Open an HTTP(S) link in this session's Operator Browser panel. */
...
function TimelineItem({
	...
	// because that is Operator's only durable write that can attach to a turn in flight,
	// but it is the user speaking and the timeline shows it that way.
	if (isSteer(item)) {
		return <SteerMessage activity={item} />;
	}
	// A plan whose turn Operator never correlated — one from before this controller
...
function groupByTurn(snapshot: ConversationSnapshot): TimelineGroup[] {
	...
	for (const item of snapshot.items) {
		if (item.turnId === undefined) {
			...
			// A provider can run a turn Operator never dispatched — a compaction, or a turn
			// resumed inside the provider's own history — and every item it emits then
			// correlates to no Operator turn. One group per item made `runsOf` see no two
