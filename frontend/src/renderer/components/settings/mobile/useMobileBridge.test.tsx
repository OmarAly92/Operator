...
function Harness({ active = true }: { active?: boolean }) {
	const bridge = useMobileBridge(active);
...
function openEvents() {
	return captureRendererEvent.mock.calls.filter((c) => c[0] === "opr.renderer.mobile_connect_opened");
}

function toggleEvents() {
	return captureRendererEvent.mock.calls.filter((c) => c[0] === "opr.renderer.mobile_bridge_toggled");
}

describe("useMobileBridge telemetry", () => {
	beforeEach(() => {
	...
	test("does not re-report the open when the bridge is toggled on", async () => {
		...
		await waitFor(() => expect(screen.getByRole("switch", { name: "Enable mobile" })).toBeChecked());
