...
// OPERATOR_PORT is only a request — the daemon may bind a different port (port 0,
...
export type RunFileInfo = {
	 ...
	 * OPERATOR_KEEP_DAEMON (stays alive across app quit, never re-linked);
	 * undefined/empty = headless `opr start` daemon.
	 */
	owner?: string;
...
/**
 * Where the daemon writes running.json when OPERATOR_RUN_FILE is unset. Matches
 * backend/internal/config's canonical Operator home default so the supervisor reads
...
export function defaultRunFilePath(
	...
	return joinPath(homeDir, ".operator", "running.json");
}
