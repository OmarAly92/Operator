...
//
// (2) is the defensive backstop for issue #367: a standalone `opr daemon` may be
...
// The daemon's default bind port (backend/internal/config). OPERATOR_PORT overrides it.
export const DEFAULT_DAEMON_PORT = 3001;
// The `service` field every genuine Operator daemon stamps on its health payloads.
export const DAEMON_SERVICE_NAME = "operator-daemon";
...
/**
 * The port a freshly spawned daemon is expected to bind: OPERATOR_PORT when set and
...
export function expectedDaemonPort(env: Record<string, string | undefined>): number {
	const configured = env.OPERATOR_PORT ? Number(env.OPERATOR_PORT) : NaN;
