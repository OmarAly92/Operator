...
/**
 * The marker the desktop app writes under ~/.operator on every launch (spec §5).
 * It is the fast-path hint `opr start` reads to locate the installed bundle.
...
/** File name of the marker under the ~/.operator state dir. */
export const APP_STATE_FILE_NAME = "app-state.json";

export interface WriteAppStateOptions {
	/** Directory the marker lives in (dirname of running.json, i.e. ~/.operator). */
	stateDir: string;
	 ...
	 * `opr start`'s --installed-via arg. Subsequent launches preserve the value
 ...
 * concurrent `opr start` reader never observes a partial file.
 */
...
/**
 * Write ~/.operator/app-state.json. The app is the SOLE writer (invariant 3) and
 ...
 * then an atomic rename, so a concurrent `opr start` reader never observes a
 * partial file.
 *
