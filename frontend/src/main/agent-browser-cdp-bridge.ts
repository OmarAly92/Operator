...
export class AgentBrowserCDPBridge {
	...
	async close(): Promise<void> {
		...
		for (const socket of this.server.clients) socket.close(1001, "Operator browser session closed");
		...
		switch (request.method) {
			case "Browser.getVersion":
				return {
					...
					revision: "",
					userAgent: "Operator agent-browser bridge",
			...
			case "Schema.getDomains":
				return { domains: [] };
			case "Browser.close":
				throw new Error("Browser.close is not permitted for Operator-owned previews");
			default:
	...
	private attachTarget(connection: ConnectionContext, target: AgentBrowserTarget): string {
		const sessionId = `opr-${randomUUID()}`;
		...
		physical.detachListener = () => {
			for (const client of physical.clients.values()) {
				this.send(client.connection.socket, {
					...
					params: { reason: "Operator page debugger was released" },
				...
				client.connection.socket.close(1012, "Operator page debugger was released");
			}
	...
	private requireTarget(targetId: string | undefined): AgentBrowserTarget {
		...
		if (!target) throw new Error("Target is outside this Operator worker");
		return target;
	}
...
function assertSafeTargetMethod(method: string, params: Record<string, unknown> | undefined): void {
	...
	if (
		...
		throw new Error(`CDP method is not permitted by Operator: ${method}`);
	}
}
