...
describe("writeAppStateMarker", () => {
	...
	beforeEach(async () => {
		dir = await mkdtemp(path.join(os.tmpdir(), "opr-app-state-"));
	});
	...
	it("first write sets installedAt + installSource from installedVia", async () => {
		...
		expect(m.appPath).toBe("/Applications/Operator.app");
	...
	it("second write PRESERVES installedAt/installSource and updates appPath/version/lastReconciledAt", async () => {
		await writeAppStateMarker({
			stateDir: dir,
			appPath: "/tmp/staging/Operator.app",
			version: "0.0.0",
			installedVia: "npm-bootstrap",
...
describe("migration marker", () => {
	const dirs: string[] = [];
	async function tmp() {
		const dir = await mkdtemp(path.join(os.tmpdir(), "opr-appstate-"));
		dirs.push(dir);
		return dir;
	}
