 ...
 * the OPERATOR_KEEP_DAEMON env var. When set to an explicit truthy value ("1",
 ...
 * an explicit `opr stop`. Any other value — including conventional falsy ones
 ...
 */
export function keepDaemonAlive(env: { OPERATOR_KEEP_DAEMON?: string }): boolean {
	const raw = env.OPERATOR_KEEP_DAEMON?.trim().toLowerCase();
 ...
 * stay unlinked even when the app is later reopened without OPERATOR_KEEP_DAEMON).
 * Only a normal app-owned daemon ("app") is linked; a keep-alive daemon
 * ("persistent") and headless `opr start` daemons (owner unset/empty) stay
