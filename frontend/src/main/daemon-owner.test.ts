...
describe("shouldLinkOnAttach", () => {
	...
	it("returns false when owner is undefined (headless opr start)", () => {
	...
	// OPERATOR_KEEP_DAEMON is stamped owner:"persistent" in running.json. A LATER
	// launch of the app — which may have OPERATOR_KEEP_DAEMON unset — must NOT
	...
	it("does NOT re-link a persistent daemon on attach, even when OPERATOR_KEEP_DAEMON is unset now", () => {
		expect(shouldLinkOnAttach("persistent")).toBe(false);
	});
});

describe("keepDaemonAlive", () => {
	it("returns false when OPERATOR_KEEP_DAEMON is unset", () => {
		expect(keepDaemonAlive({})).toBe(false);
	});

	it("returns false when OPERATOR_KEEP_DAEMON is empty", () => {
		expect(keepDaemonAlive({ OPERATOR_KEEP_DAEMON: "" })).toBe(false);
	});

	it.each(["1", "true", "TRUE", "yes", "on", "ON", "Yes"])("returns true for truthy value %j", (value) => {
		expect(keepDaemonAlive({ OPERATOR_KEEP_DAEMON: value })).toBe(true);
	});
	...
	it.each(["0", "false", "FALSE", "off", "OFF", "no", "No"])("returns false for conventional off value %j", (value) => {
		expect(keepDaemonAlive({ OPERATOR_KEEP_DAEMON: value })).toBe(false);
	});
	...
	it("trims surrounding whitespace before evaluating", () => {
		expect(keepDaemonAlive({ OPERATOR_KEEP_DAEMON: "  0  " })).toBe(false);
		expect(keepDaemonAlive({ OPERATOR_KEEP_DAEMON: "  1  " })).toBe(true);
	});
});
