...
describe("isAllowedAppExternalURL", () => {
	it("allows web and mail handoff URLs from the app renderer", () => {
		expect(isAllowedAppExternalURL("https://github.com/OmarAly92/operator/issues/new")).toBe(true);
		expect(isAllowedAppExternalURL("http://localhost:5173/help")).toBe(true);
		expect(isAllowedAppExternalURL("mailto:support@operator.example.com?subject=Operator%20feedback")).toBe(true);
	});
	...
	it("opens allowed URLs through the native shell opener", async () => {
		...
		await openAllowedAppExternalURL("mailto:support@operator.example.com?subject=Operator%20feedback", { openExternal });

		expect(openExternal).toHaveBeenCalledWith("mailto:support@operator.example.com?subject=Operator%20feedback");
	});
