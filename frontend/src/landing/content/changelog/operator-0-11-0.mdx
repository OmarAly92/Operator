---
title: "Operator 0.11.0 — The Floor"
...
- **CLI**: `opr agent ls` with install and auth status, `opr spawn` resolving project context instead of demanding `--project`, `--kind worker|orchestrator`, `opr review stop|restart`, `opr pr` (#2371, #2416, #2669, #2673, #3043)
- **Settings and first run** rebuilt — redesigned settings with the PR board folded into the per-session inspector, onboarding replacing four empty kanban columns, and a first-run scratch project so there's somewhere to try Operator immediately (#2432, #2797, #2952, #2348)
...
**Lifecycle and coordination.** Killed sessions stay dead. Worker-idle completions reach a busy orchestrator through a durable outbox instead of being dropped. Orchestrator replacement goes through a safe canonical branch handoff with stale worktree recovery. Operator no longer nudges a session that is genuinely paused on a permission dialog. (#2319, #2320, #2357, #2431, #2530, #2600, #2794, #2798, #2799, #2836, #3072)

**Daemon failures explain themselves.** Start failures surface the real error with recovery instructions and copyable output, as a floating toast rather than a banner that shoves the board around. `opr spawn --issue` without a GitHub token no longer panics the daemon. (#2690, #2871, #2972, #3017)
...
Docs and the landing site now deploy to [operator.example.com](https://operator.example.com) via GitHub Pages, with a privacy policy, a PR-wise changelog, platform-detecting download links, and a contributing guide. (#2308, #2385, #2605, #2757, #2826, #2837, #2846, #2913)
...
49 distinct authors landed work in this range, and 30 of them landed four commits or fewer. That shape is the point: a first PR to Operator is a realistic weekend.
...
**Full changelog:** https://github.com/OmarAly92/operator/compare/v0.10.0...v0.11.0
