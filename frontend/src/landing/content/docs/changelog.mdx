---
title: Changelog
description: Where to find current Operator desktop and mobile release notes.
---

Operator now ships continuously through GitHub Releases. The release page is the source of truth for desktop versions, supported artifacts, changes, and upgrade notes.

The desktop app checks for updates automatically. Open **Settings → Updates** to see the installed version or check manually. The optional CLI also reports its own version with `opr version`.

- [Current desktop releases](https://github.com/OmarAly92/operator/releases)
- [Current source history](https://github.com/OmarAly92/operator/commits/master)
...
- Launched the website and docs at [operator.example.com](https://operator.example.com), with setup guides, API reference material, and plugin documentation in one place.
...
- Hardened the CLI with install-aware `opr update`, startup update notifications, doctor version checks, JSON output for `opr session ls`, config error handling, version mismatch fixes, and default filtering for terminated sessions.
- Moved terminals and sessions onto a single multiplexed WebSocket connection, reducing connection churn and resource leaks.
- Added prompt-driven spawn: `opr spawn` can now create sessions from freeform tasks without requiring a tracker issue.
...
- Renamed all packages from `@composio` to `@operator-dev` for a cleaner ecosystem identity.
