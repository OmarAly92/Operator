...
Electron app toasts are a presentation of the same durable records while the desktop is running. There is no `opr notify`, `opr setup`, routing preset, JSONL notification store, or external notifier configuration in the current CLI.
