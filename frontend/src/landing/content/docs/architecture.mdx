...
Operator is a local, single-user system built around a long-running Go daemon. The Electron/React desktop app, optional CLI, and Expo mobile app are thin clients over the daemon's HTTP, SSE, and terminal interfaces.
...
All application state remains under `~/.operator`, including the run file, managed worktrees, Electron data, and mobile configuration. `OPERATOR_DATA_DIR` and `OPERATOR_RUN_FILE` provide advanced overrides for daemon storage and discovery.
