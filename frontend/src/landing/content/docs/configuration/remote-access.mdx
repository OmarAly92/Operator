...
<Callout type="info">
	Chat and Terminal UI are two controllers for the same Operator session. Switching does not create a new worktree, but a first switch from Terminal UI to Chat does not reconstruct old terminal screen output as structured messages.
</Callout>
