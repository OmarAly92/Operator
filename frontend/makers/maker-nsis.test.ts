...
const makeOptions = {
	dir: "/tmp/app/Operator-win32-x64",
...
describe("MakerNSIS", () => {
	...
	it("builds an nsis target for the requested arch and forwards config", async () => {
		const maker = new MakerNSIS({ appId: "dev.operator.desktop", icon: "assets/icon.ico" }, ["win32"]);
		...
		expect(artifacts).toEqual(["/out/make/Operator Setup.exe"]);
	...
	it("forwards executableName so the Start menu shortcut targets the real binary (#2414)", async () => {
		const maker = new MakerNSIS(
			{ appId: "dev.operator.desktop", executableName: "operator", icon: "assets/icon.ico" },
			["win32"],
		);
		await maker.prepareConfig(makeOptions.targetArch);
		...
		// It must match Forge's packaged "operator.exe", not the
		// "Operator.exe" it would infer from productName.
		expect(options.config.win.executableName).toBe("operator");
