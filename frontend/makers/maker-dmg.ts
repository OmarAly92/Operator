...
export default class MakerDMG extends MakerBase<MakerDMGConfig> {
	...
	async make({ dir, makeDir, targetArch, appName }: MakerOptions): Promise<string[]> {
		...
		// "Operator.app/Operator.app", an outer bundle with no
...
// an OPERATOR_NOTARY_PROFILE notarytool keychain profile locally, or the App Store
// Connect API key trio in CI.
function resolveNotaryArgs(env: NodeJS.ProcessEnv): string[] | undefined {
	if (env.OPERATOR_NOTARY_PROFILE) return ["--keychain-profile", env.OPERATOR_NOTARY_PROFILE];
...
export async function sealDmg(dmgPath: string, env: NodeJS.ProcessEnv = process.env): Promise<boolean> {
	...
	if (!notaryArgs) {
		throw new Error(
			`[dmg] a signing identity is set but no notarization credentials are (OPERATOR_NOTARY_PROFILE, or the ` +
