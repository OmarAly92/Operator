...
| `app/(tabs)/index.tsx` | `lib/feature/sessions/presentation/sessions_screen/ui/widgets/sessions_body.dart` | The Kanban tab, plus `session_actions_sheet.dart` for kill/restore/resume, and `sessions_stats_row.dart` — where tapping a stat jumps the board to that section (`jumpTo`). **The first pass of this sweep missed the jump**, reading `index.tsx:82`'s `haptics.select()` as a filter row; it was caught at M6 Task 9 by reading the source rather than the plan, and the feature was built then. A zero-count stat is deliberately silent and inert, as in RN — the haptic fires inside the guard, not on the press. |
