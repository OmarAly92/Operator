...
- **`worktree`** (the default everywhere except the two manual composer surfaces) — a `git worktree` on a generated branch under the managed root, via `adapters/workspace/gitworktree`. `opr session spawn` and every auto-review reviewer-driven spawn always get this.
