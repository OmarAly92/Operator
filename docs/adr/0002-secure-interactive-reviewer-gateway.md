...
Operator's reviewer terminal. A reusable boundary must preserve the real TUI while
...
- Each reviewer receives a private neutral working directory, configuration,
  ...
  `OPERATOR_DATA_DIR/reviewer-runtime/<reviewer-id>`. The project checkout is never its
  working directory.
- The immutable manifest binds a reviewer and worker session to exact review-run
  ids, GitHub PR URLs, target/base object ids, and Operator-owned hidden prompt files.
- Source access is structured: list pinned-tree paths, read a pinned blob, bounded
  ...
  by Operator, with no shell, caller refs, hooks, external diff, textconv, pager, system
  configuration, or optional locks.
- Side effects are structured: post a review only to the manifest PR/commit and
  submit only manifest run ids for the manifest worker. Operator selects absolute `git`,
  `gh`, and `opr` binaries; payloads use stdin and fixed argv. The TUI never receives
  an arbitrary-command primitive.
- Prompt reads resolve symlinks and remain inside the Operator prompt root. Repository
  paths reject absolute paths, traversal, option injection, NUL, and newlines.
- The gateway uses the existing loopback `opr review submit` flow and does not alter
...
- macOS/Linux tmux: launch the TUI through an Operator-owned sandbox process, mount the
  ...
  and Operator loopback submission endpoint.
...
5. GitHub posting and Operator submission work through fixed operations and cannot be
