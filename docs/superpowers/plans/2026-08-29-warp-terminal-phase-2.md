...
**Known gap, deliberately named.** Completions are Phase 3; `editor` must not import `completions`, and the boundary checker already enforces it. Task 6's keymap maps `Tab` to `accept-suggestion` rather than leaving it unbound, so Phase 3 changes one branch rather than retrofitting a key.
