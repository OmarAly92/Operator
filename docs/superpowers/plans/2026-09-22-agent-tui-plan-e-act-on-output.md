...
- **The link table against the budget, stated explicitly.** `TerminalCore::memory_stats` and `Parser::trim_to` both weigh only content plus styles, so the registry is outside `Limits { bytes: 128 MiB }` and is never trimmed. Report `core.hyperlink_count()` and the table's byte size on the act-probe (which emits one OSC 8) and on `claude-long-50k` (which emits none), and state the worst case the caps permit as a fraction of the budget: `HyperlinkRegistry` holds each URI twice (`by_link` map key, `by_id` vec element), so 2 × 4096 × 2083 B ≈ 17.1 MB ≈ 12.7 % of 128 MiB before `HashMap` overhead, reachable only by a program printing 4096 distinct maximal URIs, and held for the life of the core. If a real session's table is anything but negligible, that is the finding. Report the MEASURED table size from `core.hyperlink_count()` and its byte size alongside this corrected bound — never the plan's own arithmetic.
...
`by_link` map key and the `by_id` vec element) above its 128 MiB budget for its
lifetime; a reopened
