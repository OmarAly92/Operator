...
- **Renderer core and mirror memory at the fixture's 60k rows, each
  ...
  Go mirror wasm memory: 4,128,768 bytes (~3.94 MiB), unchanged from Plan A —
  this number comes from `TestAgentSessionReplayReport`, whose own mirror
  instance is hardcoded to a 1,000-row `Limits` independent of the product's
  200k-row `mirrorLimits`, so it does not exercise the mirror at 60k rows;
  it is under budget regardless, and the same caveat already applied to
  Plan A's number in this row.
...
All five rows Plan B owns pass. No code was changed to make any number
above hit its target — the numbers here are exactly what the commands in
