...
| main-thread block when a 2 MB tool result arrives in one mux message | `PerformanceObserver({ entryTypes: ['longtask'] })` | `claude-long-50k`: a 2 MiB feed cost 28.9ms of `core.feed`; `PerformanceObserver` reported 0 long tasks (`longestTaskMs`: none observed) | `claude-long-50k`, measured on a fresh page (Task 8 fixed a harness bug where this used to run right after the feed-cost pass, so "a 2 MiB feed" was actually whatever ≤1.3 MiB tail was left before EOF — the 28.9ms Today figure understates a genuine 2 MiB feed): a real fresh 2 MiB synchronous `core.feed` costs 57.5ms, 0 long tasks. The **queued** path Task 6 built (`core.enqueue` + rAF-driven `core.drain`) parses the same 2 MiB across 15 `requestAnimationFrame` slices, 163.4ms total (~10.9ms/frame, under the 12ms `FEED_BUDGET_MS`), 0 long tasks — this is what `bench:agent:gate` checks (`queued.longestTaskMs > 16` fails the gate; none observed) |
...
multi-paint frames of 120, also now 0), queued 2 MiB longest task: no long
task observed, drained over 15 rAF frames / 163.4ms total (was 28.9ms
synchronous — but that Today figure measured a partial end-of-fixture tail,
not a genuine 2 MiB chunk, a harness bug fixed in this task; a fresh 2 MiB
synchronous feed now measures 57.5ms, still 0 long tasks); every other row is
Plan B's target and is unchanged within run-to-run noise.
