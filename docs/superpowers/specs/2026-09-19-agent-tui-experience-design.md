...
Plan D landed 2026-09-22, measured on `agent-tui-plan-d-text-and-glyphs` HEAD
(`36cb87c0b`) after both wasm artifacts and the daemon were rebuilt. Every
visible change is behind `RendererFeatures`
(`packages/terminal/ts/renderer-dom/src/features.ts`), set through
`DomBlockRenderer.setFeatures` and the `features` prop of `TerminalSurface`;
Operator passes nothing, so every flag is at its default:
...
| `widthCache` | `false` | per-cluster letter-spacing (Task 11: landed — needed per `EVIDENCE.json`'s `wideDriftPx`/`cjkDriftPx`, evidence in `EVIDENCE-widthCache.json`) | `…/feature-widthCache/` |
| `boxDrawing` | `false` | procedural box glyphs (Task 10: not needed — `boxGapPx` = 0, `EVIDENCE.json`, measured 2026-09-22) | n/a — never implemented |
...
(measured 2026-09-21, cell 8.4375×16.796875 px): box drawing — `boxGapPx` = 0
→ not needed; width cache — `wideDriftPx` = -5.36, `cjkDriftPx` = -28.38 →
needed. `seqDriftPx` = -46.36 px with graphemes off (`EVIDENCE.json`) and
-4.22 px with graphemes on (`EVIDENCE-graphemes.json`).

Task 10 (box drawing) was skipped, not implemented: Task 1's glyph-probe
evidence showed no hairline gap to fill (`boxGapPx` = 0), so the brief's own
"only if hairline gaps" condition was never met. `RendererFeatures.boxDrawing`
exists as a flag name the plumbing accepts and threads through, but no
renderer code reads it — per the brief, a name the plumbing knows but no code
reads is still a valid flag, so it stays in the "every flag on" measurement
below rather than being dropped from the list; there is nothing behind it to
turn on. Task 11 (width cache) *was* implemented and landed (commits through
`36cb87c0b`); its own evidence file is `EVIDENCE-widthCache.json`.

**A known, pre-existing bug in `bench/agent-session/glyph-probe.mjs`**
(outside every task's file list, so not fixed here, only documented):
`MARKER_COLUMN = 40` assumes the probe's `|` marker sits at 0-based column 40,
but the fixture reaches it via `\x1b[40G` (CSI CHA, 1-based per ECMA-48), so
the true marker sits at 0-based cell 39 — a constant ~+1-cell (~8.4px) bias
baked into every `EVIDENCE*.json` this plan has produced (Task 1's original
gate, Task 5's `EVIDENCE-graphemes.json`, Task 11's
`EVIDENCE-widthCache.json`, and the two measurements in this task). It does
not change any needed/not-needed verdict — every gap that reads "needed"
clears the 1px threshold either way — but it does distort the reported
magnitudes. See `TERMINAL.md` §5.

**Flags-off rows against Plan C:** feed() cost, feed()+snapshot() cost,
paints/s, row nodes/paint, torn frames, scroll coverage/trim, reopen
first-paint timing, feel-gate pixel diff, selection repaint and the
slow-link-burst tests all read unchanged within run-to-run noise. **One row
regressed: renderer core and mirror wasm memory at 60k rows.** Flags off,
this HEAD reads 14,680,064 bytes (~14.00 MiB) for the renderer core, up from
Plan C's 8,192,000–9,175,040 bytes (~7.81–8.75 MiB) — reproduced identically
across three separate runs, not noise. The Go mirror moved the same way:
14,352,384 bytes (~13.69 MiB) at `mirrorLimits`, up from Plan C's 9,633,792
bytes (~9.19 MiB), and 5,636,096 bytes (~5.37 MiB) for the reopen-replay
probe's capped mirror, up from Plan C's 4,128,768 bytes (~3.94 MiB). This
tracks with Tasks 6–7 replacing the scalar-width TS tables with exported
per-cluster cell spans (`CELL_SPAN_WORDS`) and Task 3's exported style runs —
both are now always computed and exported in every snapshot and history
chunk, independent of whether `graphemes`/`attributes` are turned on at the
renderer. Both figures remain far under the Part 1.4 budget of < 128 MiB each
(14.00 MiB is 10.9% of it), so this is reported as a real cost of Plan D's
architecture, not tuned away, and not a failure against the < 128 MiB target.

**Flags-on cost:** with every flag on
(`attributes=warp,graphemes,cursorContrast,cursorHollowUnfocused,widthCache,boxDrawing`),
`feed()` at 50k rows read 0.20ms (20 samples) against 0.10ms flags off;
`feed()`+`snapshot()` at 50k read 0.20ms against 0.10ms flags off; DOM nodes
per spinner paint rose from 73.50 to 77.30 (row nodes per paint unchanged at
10.19 — the added nodes are attribute-span wrappers, not new rows); idle-pane
task time read 0.944s against 1.293s flags off (both inside the same noisy
0.79–1.30s spread this branch and Plan C have shown, not a clear win or
cost); renderer core memory was unchanged at 14,680,064 bytes flags on vs
flags off — the memory cost above is paid by the always-on export
infrastructure, not by which flags are set at runtime.
