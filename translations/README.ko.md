...
<img src="../docs/assets/readme/dashboard.png" alt="병렬 코딩 에이전트 세션을 보여주는 Operator 대시보드" width="100%" />
</div>
...
Operator는 AI 코딩 에이전트를 병렬로 실행하기 위한 메타 하네스 에이전트 IDE입니다. Claude Code, Codex, Cursor, Kimi Code, opencode 등 터미널 기반 에이전트에 공유 워크스페이스를 제공하여 세션, 터미널, 브랜치, 풀 리퀘스트, 피드백 루프를 한곳에서 감독할 수 있습니다.

코딩은 여전히 에이전트가 수행합니다. Operator는 그 주변 하네스를 제공합니다: 격리된 워크스페이스, 라이브 터미널 접근, 세션 상태, PR 인식, 그리고 CI 실패·리뷰 코멘트·머지 충돌을 올바른 에이전트로 자동 전달하는 루프입니다. 수많은 에이전트 터미널을 수동으로 조율하는 대신, Operator는 병렬 에이전트 작업을 관리 가능한 워크플로로 바꿉니다.

## 왜 Operator인가?

AI 코딩 에이전트는 병렬로 일할 때 훨씬 더 유용해지지만, 병렬 작업은 금방 혼란스러워집니다. 브랜치가 겹치고, 터미널을 잃고, CI 실패를 추적해야 하며, 리뷰 코멘트에 답하고, 머지 충돌을 올바른 워커에게 전달해야 합니다.

Operator는 그 루프를 가시적이고 관리 가능하게 유지하도록 설계되었습니다. 다음과 같은 도움을 줍니다:
...
높은 수준에서 Operator는 간단한 루프를 따릅니다:

1. 에이전트가 작업할 프로젝트를 추가합니다.
2. 데스크톱 앱 또는 CLI에서 하나 이상의 세션을 시작합니다.
3. Operator가 각 세션에 대해 격리된 git worktree를 만듭니다.
4. Operator가 해당 세션의 터미널 런타임에서 선택한 코딩 에이전트를 실행합니다.
5. 로컬 데몬이 세션 상태, 터미널 활동, 풀 리퀘스트, CI, 리뷰 피드백을 감시합니다.
6. 데스크톱 앱과 CLI가 현재 상태를 보여주고, 올바른 세션에 후속 지시를 보낼 수 있게 합니다.

결과는 에이전틱 코딩을 위한 로컬 제어 계층입니다. 코딩은 에이전트가 하고, Operator가 워크스페이스·상태·터미널·피드백 루프를 정리합니다.
...
<table>
  <tr>
    ...
    <td width="64%">
      <img src="../docs/assets/readme/dashboard.png" alt="여러 병렬 세션을 보여주는 Operator 보드" />
    </td>
  </tr>
  <tr>
    <td width="36%">
    ...
    <td width="64%">
      <img src="../docs/assets/readme/session-terminal.png" alt="Operator 내부의 세션 터미널" />
    </td>
  </tr>
  <tr>
    <td width="36%">
...
Operator는 23개의 워커 에이전트 하네스 어댑터를 제공합니다:
...
설치 후 Operator를 열고 Operator가 관리할 저장소를 지정하세요. 데스크톱 앱이 데몬을 실행하므로 CLI는 필요하지 않습니다. 설치된 데스크톱 빌드는 실행 시와 실행 중 주기적으로 업데이트를 확인합니다. 에이전트 CLI 설정 및 문제 해결은 [설치 가이드](https://operator.example.com/docs/installation)를 참고하세요.

<details>
<summary>npm으로 설치 (레거시 CLI, 더 이상 권장하지 않음)</summary>

npm은 여전히 동작하지만 더 이상 권장하지 않습니다. `0.10.0`이 npm에 게시된 최종 버전이며, `@operator-dev/opr` 패키지는 동결되어 추가 업데이트를 받지 않습니다. PATH에 `opr` CLI가 있는 기존 사용자를 위해 유지됩니다. `opr start`는 위에 링크된 것과 동일한 데스크톱 빌드를 가져와 엽니다. 새 설치에서는 데스크톱 다운로드를 우선하세요.
...
Operator는 제품 도입 현황을 파악하고 개선하기 위해 PII와 프로젝트 콘텐츠를 제외하도록 설계된 개인정보 보호 중심의 제품 사용 및 안정성 지표를 활용합니다. [텔레메트리와 개인정보 보호에 대해 자세히 알아보세요](../docs/telemetry.md).
