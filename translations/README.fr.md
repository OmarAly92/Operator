...
<img src="../docs/assets/readme/dashboard.png" alt="Tableau de bord Operator montrant des sessions d'agents de codage en parallèle" width="100%" />
</div>
...
Operator est conçu pour garder cette boucle visible et gérable. Il vous aide à :
...
À haut niveau, Operator suit une boucle simple :
...
3. Operator crée un git worktree isolé pour chaque session.
4. Operator lance l'agent de codage sélectionné dans le runtime terminal de cette session.
...
Le résultat est une couche de contrôle locale pour le codage agentique : les agents codent toujours, tandis qu'Operator organise leurs espaces de travail, statuts, terminaux et boucles de feedback.
...
<table>
  <tr>
    ...
    <td width="64%">
      <img src="../docs/assets/readme/dashboard.png" alt="Tableau Operator avec plusieurs sessions en parallèle" />
    </td>
  </tr>
  <tr>
    <td width="36%">
    ...
    <td width="64%">
      <img src="../docs/assets/readme/session-terminal.png" alt="Terminal de session dans Operator" />
    </td>
  </tr>
  <tr>
    <td width="36%">
...
Operator inclut des adaptateurs pour 23 harnesses d'agents worker :
...
**S'il tourne dans un terminal, il tourne sur Operator.**
...
Après l'installation, ouvrez Operator et pointez-le vers le dépôt que vous voulez qu'Operator gère. L'application de bureau lance le daemon pour vous, aucune CLI n'est requise. Les builds de bureau installées vérifient les mises à jour au lancement et périodiquement pendant l'exécution. Consultez le [guide d'installation](https://operator.example.com/docs/installation) pour la configuration des CLI d'agents et le dépannage.

<details>
<summary>Installer via npm (CLI legacy, plus recommandé)</summary>

npm fonctionne encore mais n'est plus recommandé. `0.10.0` est la dernière version publiée sur npm, et le paquet `@operator-dev/opr` est figé et ne recevra plus de mises à jour. Il reste disponible pour les utilisateurs existants qui ont la CLI `opr` dans leur PATH ; `opr start` récupère et ouvre la même build de bureau liée ci-dessus. Pour toute nouvelle installation, préférez le téléchargement de bureau.
...
| [docs/STATUS.md](../docs/STATUS.md)                                 | Ce qui est actuellement livré sur `master` et ce qui est encore en cours.                    |
| [docs/stack.md](../docs/stack.md)                                   | Décisions de bibliothèques, runtime et dépendances.                                          |

## Télémétrie anonyme

Operator utilise des mesures d'usage et de fiabilité respectueuses de la vie privée — conçues pour exclure les PII et le contenu des projets — afin de mesurer l'adoption et d'améliorer le produit. [En savoir plus sur la télémétrie et la confidentialité](../docs/telemetry.md).
