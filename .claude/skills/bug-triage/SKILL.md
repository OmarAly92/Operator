...
**Required (2 of 4):** OS/shell/runtime, Operator version (`opr --version`), reproducibility (consistent vs intermittent), reproduction steps
...
```bash
# Environment
opr --version && node --version && echo $SHELL && uname -a
cat operator.yaml
cat ~/.operator/running.json
...
# Operator event log — structured timeline
opr events list --limit 50                          # recent events
opr events list --session opr-5 --limit 100          # filter by session
opr events list --log-level error --since 1h        # errors only
opr events search "spawn failed"                    # full-text search
opr events stats                                    # counts by kind/source

# Session state files
cat ~/.operator/projects/*/sessions/*.json | python3 -m json.tool
```
...
```
Example: [#1391](https://github.com/OmarAly92/operator/issues/1391) traced a mobile layout break to a `display: flex` → `display: grid` change.
...
Operator runs on **Windows, macOS, Linux** as first-class targets. If env info indicates Windows (or is unknown), check for these patterns:

- **Path separators** — hardcoded `/` or `\` breaks cross-platform
- **Shell syntax** — PowerShell lacks `&&`, `$VAR`, `$(cat ...)`, `/dev/null`, here-docs
- **`process.platform === "win32"` inline** — must use `isWindows()` from `@operator-dev/opr-core`
- **`process.kill(-pid)`** — POSIX-only; use `killProcessTree()`
- **Named pipes vs Unix sockets** — Windows uses `\\.\pipe\opr-pty-<id>`
...
**⛔ NEVER use placeholder URLs.** Upload BEFORE creating the issue. ([#1151](https://github.com/OmarAly92/operator/issues/1151) RCA on this pattern.)
...
Example: [PR #1608](https://github.com/OmarAly92/operator/pull/1608) was diagnosed High as xterm v6 issue — real cause was a `=` prefix on tmux `set-option`. Should have been Medium.
...
```
## Related
- [#1020](url) — stale session blocking opr start (same subsystem)
...
|-----------|---------|-----------|
| **CLI** (`opr start/stop/spawn`) | Config YAML, install method, version, OS | `packages/cli/src/commands/` |
...
```bash
mkdir -p /tmp/opr-diff/{v1,v2}
curl -sL https://registry.npmjs.org/@scope/pkg/-/pkg-OLD.tgz | tar xz -C /tmp/opr-diff/v1
curl -sL https://registry.npmjs.org/@scope/pkg/-/pkg-NEW.tgz | tar xz -C /tmp/opr-diff/v2
diff -rq /tmp/opr-diff/v1/package/ /tmp/opr-diff/v2/package/
```

Example: [PR #1608](https://github.com/OmarAly92/operator/pull/1608) — source analysis led to wrong theories, npm diff showed the only change was a `=` prefix on tmux `set-option`.
