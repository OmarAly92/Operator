# `opr` CLI end-to-end tests

These tests drive the **real `opr` binary** the way a user would — `start` →
...
touch a developer's real Operator installation.

## Two tiers

| Tier                          | What                                                                                                                                                                                                                                                                  | Where                                                |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **Comprehensive (primary)**   | A cross-platform Go suite that builds `opr` and exercises the full behaviour. Runs natively on **ubuntu + macOS + windows** — the only way to cover the OS-specific process-detach paths (`setsid` vs `CREATE_NEW_PROCESS_GROUP`) and `os.UserConfigDir()` resolution. | `backend/internal/cli/e2e_test.go` (build tag `e2e`) |
...
It builds its own `opr` binary; `git` must be on PATH (required by `doctor`).
`-v` logs each `opr` invocation and its full output, which is the audit trail you
get for free from `go test`.

**Fresh-machine install, in a clean container:**

```bash
docker build -f test/cli/Dockerfile -t opr-cli-smoke .
docker run --rm --init opr-cli-smoke
```
...
**not** create/migrate `opr.db`) · `TestE2E_StatusStopped` (stopped + idempotent
...
- Deeper per-OS path assertions (state resolves under the OS-native config dir
  when `OPERATOR_RUN_FILE`/`OPERATOR_DATA_DIR` are unset) fit best as unit tests in
  `internal/config`.
