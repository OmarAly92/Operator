use vt_core::{CellStyle, CoreError, StyleCode, TerminalCore};
...
fn parses_utf8_crlf_wrap_and_sgr_into_flat_runs() {
    ...
    assert_eq!(
        ...
        &[
            (3, CellStyle::from_fg(StyleCode::ansi(1))),
            (9, CellStyle::from_fg(StyleCode::DEFAULT)),
        ]
    );
    assert_eq!(
        snapshot.row_style_pairs(1),
        &[(12, CellStyle::from_fg(StyleCode::DEFAULT))]
    );
}
...
fn surviving_first_row_begins_inside_a_style_run() {
    ...
    assert_eq!(
        snapshot.row_style_pairs(0),
        &[
            (2, CellStyle::from_fg(StyleCode::ansi(1))),
            (3, CellStyle::from_fg(StyleCode::DEFAULT)),
        ]
    );
}
...
fn extended_colour_parameters_never_leak_into_the_foreground() {
    ...
    assert_eq!(
        ...
        &[(
            2,
            CellStyle::new(StyleCode::DEFAULT, StyleCode::indexed(31))
        )]
    );
    ...
    assert_eq!(
        snapshot.row_style_pairs(0),
        &[(1, CellStyle::from_fg(StyleCode::rgb(10, 0, 0)))]
    );
}
...
fn colon_form_extended_colour_is_self_contained() {
    ...
    assert_eq!(
        snapshot.row_style_pairs(0),
        &[(1, CellStyle::from_fg(StyleCode::ansi(2)))]
    );
}
