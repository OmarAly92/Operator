...
- renderer-dom: `RendererFeatures.widthCache` measures each non-ASCII cluster once per font variant (xterm.js `WidthCache`) and applies `letter-spacing` so a fallback glyph wider or narrower than its cells no longer shifts the rest of the row; the baseline showed -5.36 / -28.38 px of drift on the probe's emoji / CJK rows, now -8.36 / -7.34 px (`EVIDENCE-widthCache.json`). Default off. **Concern, reported not fixed:** `glyph-probe.mjs`'s marker column is one cell short of its own `MARKER_COLUMN` (all three marker rows place `|` at 39 cells, not 40), so the raw drift numbers above are biased by a constant ~+1 cell and both still read "needed" against the 1px gate. Re-deriving the target from the rows' actual cell count (39, not 40) shows the correction working as designed on the `wide`/`cjk` rows (+3.08 → +0.08 px, -19.94 → +1.09 px) but regressing the `seq` row's ZWJ/skin-tone/flag sequences (-37.92 → -50.89 px): letter-spacing on a span that wraps a multi-codepoint grapheme cluster (as produced by `rowClusters`) inserts space between the cluster's own code points in Chromium, not just after it — a limitation this task's brief's own reference implementation (xterm.js `WidthCache`/`DomRendererRowFactory`) shares, so it was implemented as specified rather than patched around.
