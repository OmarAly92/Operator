...
- vt-core: a character printed over one half of a wide character blanks the other half (`TERMINAL.md` §4.37). Before, `日日` then `日` at column 1 of a 4-column row left a 3-cell span; a narrow character over a lead kept the orphaned continuation. `ScreenGrid::clear_split_wide` runs before every print path. The goldens `synthetic-unicode-mix`, `synthetic-edits-styled` and `resize-running-3` were re-recorded for this change only.
