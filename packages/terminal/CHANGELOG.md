...
- editor/react: `EditorHost.onDraftChange(draft)` and `TerminalSurface`'s `onDraftChange` prop report the line editor's unsent text each time it changes (typing, submit, `setText`), so a host can tell whether a pane holds a draft.
