...
- vt-core/vt-wasm/core: the snapshot exports `spanRanges`/`cellSpans` (`CELL_SPAN_WORDS = 3`: row-relative byte `start`, `end`, cell `width`) for every cluster that is not a single width-1 scalar, on the primary and the alternate screen, through the incremental export like the style runs. Nothing reads them yet. This buffer and Task 3's style-run word are now always computed and exported regardless of which `RendererFeatures` are set: renderer/mirror wasm memory at 60k rows rose from ~8 MiB to ~14 MiB (still well under the 128 MiB budget) — see the spec's Plan D table (`docs/superpowers/specs/2026-09-19-agent-tui-experience-design.md`, "Plan D landed") for the full numbers.
