...
- editor/react/renderer-dom: Ctrl+F/B/P/N move and walk history at an owned prompt instead of reaching the shell as control characters; Cmd+F and the geometry observer read the current find bar and renderer after a rebuild; the find bar sits in a sticky zero-height anchor so it stays in view when the transcript scrolls (`TERMINAL.md` §4.40).
- vt-host: the attach replay frame brackets its settled rows — every row up to the end of the last finished, non-synthetic block, never the last row — between `OSC 7000;v=1;settled=begin ST` and `OSC 7000;v=1;settled=end ST` (the begin right after the modes, the end right after that row's CR-LF). A client that already holds those commands (Operator's shell panes seed them from the daemon's durable blocks) drops the bracketed rows and keeps only the live tail; every other client ignores the unknown key, and a frame with no finished block carries no marks. Before, a shell pane opened on a second client or after a reload showed every finished command twice: once as durable blocks with headers, then again as one header-less block of raw frame text (`vtwasm/settled_test.go`). Only the host wasm and the daemon must be rebuilt; vt-core is unchanged.
- react: the find chord is Cmd+F on macOS and Ctrl+Shift+F elsewhere, caught on `document` in the capture phase with `preventDefault` + `stopPropagation`, so it never reaches the line editor or a full-screen program. Plain Ctrl+F goes to the shell as `\x06` (readline/zle forward-char); before, Ctrl+F opened the bar and also typed `^F`, so the next command ran as `^Fsleep` (`isFindChord` in `selection-gesture.ts`).
- react: clicking inside the find bar (its input, its `.*` toggle) or any editable control in the host keeps focus there instead of moving it to the line editor (`OWNS_FOCUS`); a press or release on host chrome (`SELECTION_CHROME`) is never reported to a program tracking the mouse; keys and pastes in the find input are not sent to a full-screen program.
- renderer-dom: the find bar scrolls the current match into view when a query first finds something (as the query is typed, or when the first match arrives later), so Enter moves on from a match the user can see; later hits from streaming output do not move the view.
- vt-host: the attach replay carries the program's OSC 22 pointer shape (`ESC ] 22 ; <css> ST` after the modes), so a pane attached after a program set it shows it.
