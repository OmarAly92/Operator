...
describe("paintedRowOrigin", () => {
	it("returns the painted position of the row itself", () => {
		...
		const elements = new Map<BlockId, HTMLElement>([
			...
			["b", section([3, 4, 5, 6], 200 - 3 * 16, 40, 16)],
		]);
		expect(paintedRowOrigin(blocks, elements, 4, 16, 0)).toEqual({ left: 40, top: 216 });
	});

	it("returns the same painted position when the stable base is shifted", () => {
		const blocks = [block("a", 0, 3), block("b", 3, 4)];
		const elements = new Map<BlockId, HTMLElement>([
			["a", section([100, 101, 102], 100 - 100 * 16, 40, 16)],
			["b", section([103, 104, 105, 106], 200 - 103 * 16, 40, 16)],
		]);
		expect(paintedRowOrigin(blocks, elements, 4, 16, 100)).toEqual({ left: 40, top: 216 });
	});
	...
	it("extrapolates a row the virtualiser left unpainted", () => {
		...
		expect(paintedRowOrigin(blocks, elements, 25, 16, 0)).toEqual({ left: 8, top: 900 });
	});
	...
	it("extrapolates past the trimmed end of the last block", () => {
		...
		expect(paintedRowOrigin(blocks, elements, 5, 16, 0)).toEqual({ left: 4, top: 90 });
	});

	it("gives up when nothing is painted", () => {
		expect(paintedRowOrigin([], new Map(), 0, 16, 0)).toBeNull();
		expect(paintedRowOrigin([block("a", 0, 1)], new Map(), 0, 16, 0)).toBeNull();
		expect(
			paintedRowOrigin([block("a", 0, 1)], new Map([["a", section([], 0, 0, 16)]]), 0, 16, 0),
		).toBeNull();
	});
});
