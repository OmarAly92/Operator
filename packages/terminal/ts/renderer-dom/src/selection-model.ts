...
function expand(point: SelectionPoint, kind: SelectionKind, rowText: RowText, rowSpans: RowSpans): [Boundary, Boundary] {
	if (kind === "line") {
		return [
	...
	if (kind === "word") {
		const word = wordCellRange(rowText(point.blockId, point.row), rowSpans(point.blockId, point.row), point.column);
		return [
...
export function resolveRange(state: SelectionState, order: BlockOrder, rowText: RowText, rowSpans: RowSpans = () => []): SelectionRange | null {
	const [headStart, headEnd] = expand(state.head, state.kind, rowText, rowSpans);
	const [tailStart, tailEnd] = expand(state.tail, state.kind, rowText, rowSpans);
