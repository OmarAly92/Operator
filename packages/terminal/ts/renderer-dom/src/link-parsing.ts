...
export interface LinkPartialRange {
	index: number;
	text: string;
}
 ...
 */
const linkSuffixRegexEol = lazy<RegExp>(() => generateLinkSuffixRegex(true));
/**
 * A regex that extracts the link suffix which contains line and column information.
 */
const linkSuffixRegex = lazy<RegExp>(() => generateLinkSuffixRegex(false));
...
export function toLinkSuffix(match: RegExpExecArray | null): LinkSuffix | null {
...
function binaryInsertList(list: ParsedLink[], newItems: ParsedLink[]) {
	if (list.length === 0) {
		list.push(...newItems);
	}
...
function binaryInsert(list: ParsedLink[], newItem: ParsedLink, low: number, high: number) {
	if (list.length === 0) {
		list.push(newItem);
		return;
	}
...
function detectLinksViaSuffix(line: string): ParsedLink[] {
	...
	for (const suffix of suffixes) {
		...
		if (possiblePathMatch && possiblePathMatch.index !== undefined && possiblePathMatch.groups?.path) {
			...
			// section)
			let prefix: LinkPartialRange | undefined = undefined;
 ...
 */
const gitDiffLineRegex = lazy<RegExp>(() => new RegExp(`^[-+]{3} ${diffFilePrefix}`));
/**
 * A regex that matches filenames in lines like `diff --git a/foo b/foo` without the prefix.
 */
const gitDiffTextRegex = lazy<RegExp>(() => new RegExp(`^${diffFilePrefix}`));

function detectPathsNoSuffix(line: string, os: LinkOs): ParsedLink[] {
	const results: ParsedLink[] = [];

	const regex = new RegExp(os === "windows" ? winLocalLinkClause : unixLocalLinkClause, 'g');
	let match;
