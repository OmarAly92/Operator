...
// prebuilt Go `opr` binary for this host, then exec it transparently.
// Zero install scripts; zero third-party deps. The binary is delivered by npm
// installing only the matching `@operator-dev/opr-<platform>-<arch>` package (its
...
// `@operator-dev/opr-<platform>-<arch>` to mirror that exactly.
const platform = process.platform;
const arch = process.arch;
const pkg = `@operator-dev/opr-${platform}-${arch}`;
const binName = platform === "win32" ? "opr.exe" : "opr";
...
if (!binary) {
  process.stderr.write(
    `@operator-dev/opr: no prebuilt binary for ${platform}-${arch}.\n` +
...
if (result.error) {
  if (result.error.code === "ENOENT") {
    process.stderr.write(
      `@operator-dev/opr: binary not found at ${binary}.\n` +
        `Reinstall @operator-dev/opr to restore the platform package.\n`,
    );
  } else {
    process.stderr.write(`@operator-dev/opr: failed to run binary: ${result.error.message}\n`);
  }
  process.exit(1);
}
