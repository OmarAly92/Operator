...
export function conversationActionError(error: unknown): string {
	...
	switch (code) {
		...
		case "CHAT_AUTH_REQUIRED": return "Sign in with the agent CLI on the Operator host, then resume this session.";
		case "CHAT_DRIVER_UNAVAILABLE": return "The agent CLI is unavailable on the Operator host. Install it or open the worktree shell.";
		case "CHAT_DRIVER_INCOMPATIBLE": return "The installed agent CLI is not compatible with Operator Chat. Update it, then resume this session.";
		case "CHAT_RESUME_FAILED": return "Operator could not resume this agent. The conversation and worktree are preserved.";
