...
describe("sessionTitle", () => {
	it("prefers displayName, then issueId", () => {
		expect(sessionTitle(session({ displayName: "Fix auth", issueId: "Operator-12" }))).toBe("Fix auth");
		expect(sessionTitle(session({ issueId: "Operator-12" }))).toBe("Operator-12");
	});
