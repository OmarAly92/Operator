...
function run(count: number, startState: RateLimitState = {}, name = "opr.v2.mobile_app.feature_used", now = T0) {
...
describe("checkRateLimit", () => {
	...
	it("reopens the minute window after 60s", () => {
		...
		const later = checkRateLimit(first.state, "opr.v2.mobile_app.feature_used", T0 + 61_000);
	...
	it("enforces the daily ceiling across many minute windows", () => {
		...
		for (let i = 0; i < EVENTS_PER_NAME_PER_DAY + 50; i++) {
			const r = checkRateLimit(state, "opr.v2.mobile_app.connected", T0 + i * 30_000);
			state = r.state;
	...
	it("resets the daily counter on a new UTC day", () => {
		...
		const capped = checkRateLimit(state, "opr.v2.app.active", T0 + 5000);
		expect(capped.allowed).toBe(false);
		const nextDay = checkRateLimit(capped.state, "opr.v2.app.active", Date.parse("2026-08-08T00:01:00Z"));
...
describe("mergeRateState (restart persistence)", () => {
	...
	it("keeps the higher day count for the same day, so a restart cannot reset it", () => {
		const persisted = { "opr.v2.mobile_app.connected": { minuteStart: 0, minuteCount: 0, day: DAY, dayCount: 199 } };
		const current = { "opr.v2.mobile_app.connected": { minuteStart: 1, minuteCount: 1, day: DAY, dayCount: 1 } };
		expect(mergeRateState(persisted, current)["opr.v2.mobile_app.connected"].dayCount).toBe(199);
	});
