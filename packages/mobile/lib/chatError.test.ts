...
describe("Chat preflight errors", () => {
	...
	it("does not offer TUI for unrelated conflicts and keeps daemon detail readable", () => {
		...
		expect(chatErrorCopy(apiError("CHAT_AUTH_REQUIRED", "409 Conflict - Sign in to Claude Code on the Operator host"))).toBe(
			"Sign in to Claude Code on the Operator host",
		);
	});
});
