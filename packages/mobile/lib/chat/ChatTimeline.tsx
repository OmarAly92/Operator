...
function OriginMessage({ message }: { message: Extract<ConversationItem, { kind: "message" }> }) {
	...
	return <View style={styles.originMessage}>
		<View style={styles.originHeader}><Feather name="radio" size={11} color={t.textTertiary} /><Text style={styles.originLabel}>{message.senderLabel || (message.origin === "automation" ? "Automation" : "Operator")}</Text></View>
...
function ChangedFiles({ turn }: { turn: ConversationTurn }) {
	...
	return <View style={styles.planCard}>
		...
		</View>)}{turn.diff?.truncated ? <Text style={[styles.partial, { color: t.amber }]}>This turn changed more files than Operator lists here. Open the worktree shell for the complete diff.</Text> : null}</View> : null}
	</View>;
}
...
function ApprovalCard({ activity, busy, onDecide }: { activity: ConversationActivity; busy: boolean; onDecide(requestId: string, decisionId: string): Promise<void> }) {
	...
	return <View style={[styles.requestCard, pending && { borderColor: t.amber }]}>
		...
		}} />)}</View> : <Text style={[styles.partial, { color: t.amber }]}>The agent offered no decisions Operator can present. Open diagnostics from the host.</Text>) : <Text style={styles.partial}>Already answered. This card is kept for the record.</Text>}
...
function UserInputCard({ activity, busy, onResolve }: { activity: ConversationActivity; busy: boolean; onResolve(requestId: string, action: "accept" | "decline" | "cancel", content?: Record<string, unknown>): Promise<void> }) {
	...
	return <View style={[styles.requestCard, pending && { borderColor: t.blue }]}>
		...
		</View> : pending ? <Text style={[styles.partial, { color: t.amber }]}>This request has no provider identity, so Operator cannot answer it safely. Open diagnostics on the host.</Text> : <Text style={styles.partial}>Already answered. This card is kept for the record.</Text>}
	</View>;
}
...
function truncationNote(value: unknown): string | undefined {
	...
	return `This payload${bytes} was larger than Operator stores, so it was not kept.`;
}
