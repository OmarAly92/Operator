...
abstract class PreviewRepository {
  FutureResult<PreviewModel?> getPreview(
    String sessionId, {
    String? previewUrl,
  });
}
...
class PreviewRepositoryImp implements PreviewRepository {
  PreviewRepositoryImp(this._remoteDataSource, this._configStore);
      ...
      if (entry.isNotEmpty && config != null) {
        return Result.success(
          PreviewModel(
            entry: entry,
            url:
                '${config.httpBase}${EndPoints.sessionPreviewFile(sessionId, entry)}',
            authenticated: true,
          ),
        );
      }

      final external = mobileReachablePreviewUrl(
        previewUrl,
        config?.host ?? '',
      );
