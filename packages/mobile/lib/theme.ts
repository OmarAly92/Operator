// Mission Control palette - mirrors Operator's DESIGN.md so the phone app reads as the
...
// Operator's attention levels, in urgency order. Drives the board sections.
...
// One status maps to one dot color and short label. Mirrors Operator's getStatusSpec so
