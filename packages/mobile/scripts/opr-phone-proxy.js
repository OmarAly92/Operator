#!/usr/bin/env node
// Minimal LAN bridge for the Operator mobile app - trust-on-first-connect.
//
// The Operator daemon stays bound to localhost (127.0.0.1:3001) - fully local and
...
//   node packages/mobile/scripts/opr-phone-proxy.js         # first device pairs
//   RESET=1 node packages/mobile/scripts/opr-phone-proxy.js # forget + re-pair
//   PORT=3011 TARGET=3001 node packages/mobile/scripts/opr-phone-proxy.js
//
// Env:
//   PORT    LAN port to expose      (default 3011)
//   TARGET  loopback daemon port    (default 3001)
//   STATE   pairing file path       (default ~/.operator/phone-allow.json)
...
const STATE = process.env.STATE || path.join(os.homedir(), ".operator", "phone-allow.json");
...
server.listen(PORT, "0.0.0.0", () => {
	console.log(
		`Operator phone bridge: 0.0.0.0:${PORT} -> 127.0.0.1:${TARGET}  | ` +
