...
void main() {
  group('data source', () {
    test('reads the entry the detector found', () async {
      ...
      when(
        () => apiConsumer.get(any()),
      ).thenAnswer((_) async => _response({'entry': ' dist/index.html '}));

      final entry = (await PreviewRemoteDataSourceImp(
        apiConsumer,
      ).getPreview('s-1')).data!;
    ...
    test('treats a missing entry as an empty one', () async {
      ...
      expect(
        (await PreviewRemoteDataSourceImp(
          apiConsumer,
        ).getPreview('s-1')).data!.entry,
        isEmpty,
      );
    });
  });
  ...
  group('repository', () {
    ...
    setUp(() {
      ...
      repository = PreviewRepositoryImp(dataSource, configStore);
    ...
    test('builds the URL from our own base, not the daemon report', () async {
      when(() => dataSource.getPreview(any())).thenAnswer(
        (_) async => const GlobalResponse(
          data: PreviewEntryModel(entry: 'dist/index.html'),
        ),
      );
      ...
      expect(
        preview!.url,
        'http://10.0.0.5:3011/api/v1/sessions/s-1/preview/files/dist/index.html',
      );
    ...
    test('honours the TLS toggle when building the base', () async {
      ...
      when(() => dataSource.getPreview(any())).thenAnswer(
        (_) async =>
            const GlobalResponse(data: PreviewEntryModel(entry: 'index.html')),
      );
    ...
    test(
      'falls back to a phone-reachable dev server without forwarding auth',
      () async {
        when(() => dataSource.getPreview(any())).thenAnswer(
          (_) async => const GlobalResponse(data: PreviewEntryModel(entry: '')),
        );

        final result = await repository.getPreview(
          's-1',
    ...
    test(
      'reports no preview at all when there is neither an entry nor a dev server',
      () async {
        when(() => dataSource.getPreview(any())).thenAnswer(
          (_) async => const GlobalResponse(data: PreviewEntryModel(entry: '')),
        );
    ...
    test('preserves request failures', () async {
      when(() => dataSource.getPreview(any())).thenThrow(
        ServerFailure(error: 'offline', message: 'offline', statusCode: 503),
      );

      expect((await repository.getPreview('s-1')).isFailure, isTrue);
      verify(() => dataSource.getPreview(any())).called(1);
    });
  });
}
